...
- **第一个里程碑（Phase 0.8）**：创建课程 → 上传 PDF/图片/文本 → 转为纯文本 → Kimi 生成结构化笔记 + 重点 + 思维导图 → 前端渲染。这界定了最小底座（S1 基础 + S2 核心）—— 不要把练习、错题本、家长面板、ASR、期末功能塞进来。
- **AI 路由**：Kimi 是默认 Provider（多模态+工具调用+工程分解）；转换器输出纯文本，LLM 绝不作为主要的格式转换路径。Qwen 作为备选持续跟踪；GPT/Claude 通过中转服务作为最难推理兑底。
