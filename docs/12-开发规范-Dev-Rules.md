...
本文适用于所有会修改 `I:\ai-studybuddy` 主仓库的任务，包括文档、后端、前端、测试和脚本变更。当前单一事实来源顺序如下：
...
- S3 PRD 已在获批的 Phase 1-T03 文档任务中创建；Phase 1-T03A 数据库与 Schema、T03B 练习生成 API、T03C 限时作答与规则批改、T03D 练习前端闭环均已完成。S3 MVP 已验收，但 S3 Worker 仍未开始；S4 PRD、T04A 错题归档与 Schema、T04B 错题改错前端闭环、T05 回流规则、T06 S6 家长观察 PRD、T06A 规则报告生成、T06B 家长报告推送和 T07 S1 时间线扩展均已完成。T06B 只使用冻结脱敏快照，按 `report_key + channel` 独立去重/重试，SMTP 与飞书失败互不阻塞；每渠道对同一批次跨运行累计最多自动尝试三次，达到上限后保存本机脱敏 HTML 和固定错误摘要供人工补发；未把真实 SMTP、飞书、Provider 或计划任务注册作为常规验证依赖。T07 已扩展时间线过滤并在考试工作台展示当前课程近期活动。当前下一门禁为 T08：本机配置中心与连接验收的独立计划、审查和用户明确批准；T08 尚未实现，S5/S7 继续按各自门禁等待。
...
6. **拆分任务、逐项实现**：用当前工具的任务列表逐项推进，不批量标记完成。
...
- 正式数据目录和验证数据目录必须隔离；所有可重复验证优先使用 `I:\ai-studybuddy-tmp\runs\<task-id>`。
