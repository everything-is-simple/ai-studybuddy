...
**版本**：v0.1
**日期**：2026-07-20
**状态**：Phase 2-T01 已获用户明确批准并创建本文档；后续 T02–T06 尚未启动，必须分别独立计划、审查和获批后实施
...
| 速背卡片 | 短标题、要点、来源模块/错题引用、已看状态 | 后续 T04 设计持久化与 UI |
...
> 本节只描述概念对象，不代表 T01 创建 Schema。真实表结构、迁移版本、DTO 和 API 必须在 T02–T06 各自计划中确定。
...
- [ ] 已确认考试的工作台可在考前窗口展示 S5 冲刺入口；未确认考试不触发正式冲刺。
- [ ] 模拟卷生成只使用当前考试范围、知识模块摘要和错题/薄弱点摘要，失败时不创建空卷。
- [ ] 模拟考支持限时作答、提交、客观题批改、成绩统计、模块分析和结果回看。
- [ ] 临考速背卡片能追溯到知识模块、错题或薄弱点，不暴露无来源的 AI 编造内容。
- [ ] 冲刺计划能按剩余天数、未完成任务、练习表现和薄弱点给出每日建议。
- [ ] S5 不反写或重判 S3/S4 历史事实；掌握状态仍由 S4/T05 证据驱动。
- [ ] S5 写入的 StudyEvent 只包含脱敏摘要和 `evidence_ref`，不包含题干、答案、作答或卡片正文。
- [ ] S6 只能读取模拟次数、完成状态、趋势和冲刺活动摘要，不读取模拟卷内容或速背正文。
- [ ] AI 未配置或失败时，学生仍可进入错题、知识模块和既有练习历史做人工复习。
- [ ] 不同学期、课程和考试的冲刺数据不会混用。
...
| Phase 2-T02 | 模拟考 Schema 与生成 | ⏳ 尚未创建实施计划；不得提前实现 |
| Phase 2-T03 | 模拟考前端 | ⏳ 等待 T02 完成和独立计划 |
| Phase 2-T04 | 临考速背 | ⏳ 等待独立计划、审查和用户批准 |
| Phase 2-T05 | 冲刺计划生成 | ⏳ 等待独立计划、审查和用户批准 |
| Phase 2-T06 | 工作台“冲刺”区集成 | ⏳ 等待独立计划、审查和用户批准 |
| 后续增强 | 主观题评分、真题导入、跨学期复用、S6 冲刺摘要扩展 | 另立计划，不由本文直接授权 |

T02–T06 不因本文档创建而自动获得实施授权。每个后续任务都必须先在 `docs/04` 登记 `.plans/`，经过独立审查，并取得用户明确批准。
