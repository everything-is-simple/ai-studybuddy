...
**版本**：v0.1
**日期**：2026-07-16
**状态**：Phase 1 轻量 PRD 已创建；实现尚未开始，下一门禁为 T04A Schema 与归档的独立计划、审查和用户明确批准
...
S4 不修改 S3 的题目、原始作答或批改结果。`practice_answers.is_correct = false` 只作为只读事实输入；错题归档、薄弱点记录、前端闭环和回流规则分别留给后续独立任务实现。
...
- [ ] T04A、T04B、T05 未经各自独立计划和用户批准不得开始实现。
...
| Phase 1-T04A | `Mistake` / `WeakPoint` Schema、错误作答幂等归档、规则与测试 | 本 PRD 已合入 `master`；T04A 计划通过独立审查并获用户明确批准 |
| Phase 1-T04B | 错题列表、详情、错因确认、原题重做和工作台入口 | T04A 已验收；T04B 计划获批 |
| Phase 1-T05 | 复习排程、知识模块/任务优先级回流、掌握后降频 | T04B 已验收；T05 计划获批 |
| 后续增强 | 同类题/变题重做、AI 错因建议优化、跨学期显式复用 | MVP 稳定后另立计划 |

S5、S6、S7 不因本 PRD 创建而自动获得实施授权。S6 仍只允许在自身门禁满足后读取 S4 的脱敏聚合；S5/S7 继续按既有阶段等待。
