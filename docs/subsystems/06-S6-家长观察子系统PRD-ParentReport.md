...
**版本**：v0.1
**日期**：2026-07-17
**状态**：Phase 1-T06 轻量 PRD 已创建；T06A 报告生成与 T06B 推送渠道仍待独立计划、审查和用户明确批准
...
> 上述状态是 PRD 概念约定，不代表本轮新增数据库字段、API 类型或代码实现。
...
- 渠道发送去重应按 `report_key + channel` 处理，同一周期同一渠道不重复发送。
- 渠道失败互不阻塞：QQ SMTP 失败不影响飞书，飞书失败不影响 QQ SMTP。
- 失败重试只能重试失败渠道，不能重新生成不同事实版本来覆盖原报告。
...
- [ ] 能基于既有事实生成日报、周报、月报和考前 7/3/1 天提醒的规则报告。
- [ ] 规则报告在 AI 不可用时仍可使用。
- [ ] AI 增强只能基于脱敏规则报告，不引入新事实。
- [ ] 报告不包含资料原文、笔记正文、完整题干、完整答案、学生作答、错因正文、聊天内容、真实渠道地址或完整 UUID。
- [ ] S3/S4 只向 S6 暴露脱敏统计和趋势，不暴露题目、答案、作答或错因正文。
- [ ] 报告按 `report_key + channel` 去重，同一周期同一渠道不重复发送。
- [ ] QQ SMTP 与飞书 Webhook 渠道失败互不阻塞，可分别重试。
- [ ] 未确认考试不会触发正式考前提醒。
- [ ] 家长不能登录、不能编辑任务、不能访问学生本机页面。
...
| T06A | 规则报告生成、AI 摘要/润色、AI 失败兜底、报告内容测试 | ⏳ 待独立计划、审查和用户明确批准 |
| T06B | QQ SMTP、飞书 Webhook、渠道失败隔离、去重与推送测试 | ⏳ 待 T06A 验收后独立计划、审查和用户明确批准 |
| Phase 3 | 家长端安全、性能与更完整配置治理 | ⏳ 不属于 Phase 1 MVP 简版 |

T06A/T06B 不因本文档创建而自动获得实施授权。S5、S7 和 Phase 3 继续按既有门禁等待。
