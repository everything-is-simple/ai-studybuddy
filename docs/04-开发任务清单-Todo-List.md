...
| Phase 1   | 跑通完整学习闭环（S1+S2+S3+S4+S6 简版） | 🔄 进行中（T00/T10/T02/T03/T11/T03A/T03B/T03C/T03D/T04/T04A/T04B/T05/T06/T06A/T06B/T07/T08/T09A ✅；T09B 待主线集成，T09C–T09E 未启动） |
...
**前置条件**：Phase 0.8 T09 已通过。S1/S2/S3/S4/S6 PRD 与 Phase 1-T00–T09A 对应范围已完成；S5/S7 仍未触发。T09B 实现与任务分支验证已完成，现待主线 fast-forward 与主线复验；T09C–T09E 仍未开始，进入任一后续任务前必须重新完成各自的独立计划、审查和用户明确批准。
...
| 21 | Phase 1-T09B：每日学习首页 | ⏳ | 今日待办任务、临近考试倒计时、待质检资料、错题复习提醒、下一步行动建议；实现提交 `562a633`、独立复审和分支验证已通过，任务分支已推送；待 fast-forward 合入 `master`、主线复验与 `origin/master` 推送 |
...
| T09B | `.plans/phase1-t09b-daily-study-home-plan.md` | 已完成独立计划、复审与实施；实施分支 `codex/phase1-t09b-daily-study-home` 的实现提交 `562a633` 已推送，分支验证通过；待 fast-forward 合入 `master`、主线复验与 `origin/master` 推送 |
...
>
> **T09B 实施验证证据（2026-07-18，待主线集成）**：任务分支 `codex/phase1-t09b-daily-study-home` 已按 `.plans/phase1-t09b-daily-study-home-plan.md` 完成每日学习首页实现与独立复审。首页只复用 T09A 的 current semester 与显式 `semesterId`，以单个 ready 学期库做只读聚合，展示今日/明日任务、已有明日课程、已确认考试、待质检或转换失败资料、错题复习与确定性下一步；缺失 current semester 回到既有 `/semesters`，普通错误可重试，stale semester 交由既有应用壳恢复。未新增 migration、写 API、全局导航、课表编辑、练习历史、T09C–T09E、S5、S7、家长 Web 面板或 Phase 2/3，KaTeX 535.51 kB warning 仍是独立遗留项。实现提交 `562a633` 已推送；验证通过：`pnpm type-check`、后端/前端 build、T09B 后端真实 SQLite 集成测试 3/3、前端页面测试 4/4、前端根级测试 69/69、后端全量测试 218/218、隔离专项 Playwright E2E 1/1、文档治理与差异检查。首次全量中的一次既有 S4 子进程启动失败经单文件 6/6 与后端全量 218/218 重跑未复现，未改动 S4。当前尚未合入 `master`、尚未推送 `origin/master`，不得据此报告 T09B 主线完成。
