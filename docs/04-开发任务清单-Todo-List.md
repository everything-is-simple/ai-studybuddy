...
> 当前进度：Phase 0.5 历史组件验证、Phase 0.7 开发机验收、Phase 0.8 第一个可运行里程碑均已完成。S1 基础与 S2 核心已在主仓库实现并通过 T09 隔离复验；Phase 1 先修正协作基线，再补齐人工补文、考试/任务浏览器闭环和 Provider 健康熔断，之后再创建 S3 PRD。
...
**前置条件**：Phase 0.8 T09 已通过。S1/S2 PRD 已存在且 MVP 已实现；S3 PRD 触发条件已满足但尚未创建；S4/S5/S6/S7 仍未触发。
...
- [ ] S3 数据对象：按 S3 PRD 创建必要表；每题关联知识模块和来源，保存逐题作答、正确性、用时与得分。
- [ ] S4 ErrorFixer：按 S4 PRD 创建 `mistakes`、`weak_points`；错题记录次数/最近错误/掌握状态，薄弱点由多次证据归纳。
...
> 任务在 S5 轻量 PRD 创建后补入。Phase 1 可以借鉴“临考速背”的信息架构，但不得提前实现 S5 功能。
