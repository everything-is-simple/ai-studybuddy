...
| T02 | T02-R2 真实 Windows 目录与 ACL 证据 | 在批准机器只读取证据：config/data/logs/backups/tmp/models 的实际 ACL、继承、主体和越权风险；形成脱敏风险结论及是否需要最小 ACL 修复的建议。任何 ACL 修复须另建、另批最小任务并回滚/复验 | **必做** | ⏸️ 未完成，暂停 | T02 总计划数据/部署范围、Slice 3、§8；T02G 计划 G2/G5 与“真实操作另批”门禁 |
