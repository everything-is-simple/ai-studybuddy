...
| 4GS | T02/T04-G2：正式语义落地计划 | ✅ 计划 fresh-pass；正式同步待用户明确批准 | 仅规划将跨平台 G2 强证据语义同步至 `docs/08`、`docs/09`、S7 PRD 与任务事实；不执行隔离、不改产品、不改变 T02/T04/T05/T06 状态 |
...
> **Phase 1.5-T02/T04-G2 正式语义落地计划（2026-07-22，fresh-pass `PASS`，正式同步待用户明确批准）**：已从最新 `origin/master` `f5288f7ec0a3fe9b6310463aab7a0903f30f8cda` 创建任务分支 `codex/phase1-5-asr-g2-semantics-adoption-plan`，并登记 `.plans/phase1-5-t02-t04-asr-g2-semantics-adoption-plan.md`。本计划只定义未来将平台无关的强制出站隔离证据语义正式同步到 `docs/08`、`docs/09`、S7 PRD 与本文件的顺序、统一措辞、历史状态保留、文档验证与停止边界；本轮不修改这些 SoT，不执行 G2/G1/G3、模型/Provider/ASR/FFmpeg smoke 或任何平台隔离，不创建 `AuralConverter`，不改产品代码，不启动 T05/T06。G1/G3 既有 `PASS`、旧 G2=`BLOCKED` 历史、T02/T04=`PARTIAL`、T03=`PASS`、T05/T06 未启动均不变；能力验证、文档同步、产品接入与生产发布资格继续严格分离。
