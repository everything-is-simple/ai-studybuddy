...
**状态**：用户于 2026-07-28 明确撤销 2026-07-27 的全面暂停决定，授权重新开放 Phase 3，但只恢复“必须做且高权重”的工作；同日已明确批准按 `PHASE3-REOPEN-HIGH-WEIGHT-20260728` 从 Wave 0 开始。T02A–T02G 已各自完成主线复验并进入 `origin/master`；T02G 只完成仓库外合成夹具安全边界和 P1 修复，真实 ACL、真实备份/恢复、总体审计与其余候选工作仍未完成。Wave 0 的 T02-R1 已创建逐项实施计划，仍待独立审查与单项批准；本轮尚未开始业务实现。
...
| PHASE3-T02-R1-REAL-SECRET-SCAN-20260728 | `.plans/phase3-t02r1-real-secret-scan-signoff-plan.md` | ⚠️ 已获单项实施批准；实现和合成验证完成，独立复审不通过：批准记录可信签发与同一安全句柄读取（TOCTOU）仍为 P1；未扫描真实仓库或候选包 |
...
| T02 | T02-R1 正式仓库与实际部署包秘密扫描签收 | 仅对 Git 已跟踪文件清单和已批准、已识别候选部署包根执行受控扫描；复核命中并形成不泄露秘密值、敏感文件名或原始输出的签收证据。记录 artifact ID、构建提交短哈希、包 SHA-256 短指纹和批准时间窗口，不扫描整个工作区或未跟踪目录 | **必做** | ⚠️ Wave 0：已获 `PHASE3-T02-R1-REAL-SECRET-SCAN-20260728` 单项实施批准；签收脚本与仓库外合成验证已完成，独立复审不通过：已关闭包根先验/任意仓库入口，但批准记录可信签发与同一安全句柄读取（TOCTOU）仍为 P1。现有正式部署包 manifest 缺少身份字段，真实扫描与签收仍 fail-closed 未完成 | `PHASE3-T02-R1-REAL-SECRET-SCAN-20260728`；上位 `PHASE3-REOPEN-HIGH-WEIGHT-20260728` Wave 0；T02D 合成夹具不等于真实扫描边界 |
