...
> **Phase 1.5-T02/T04-G2 标准 Windows 实测补证计划（2026-07-23，fresh-pass `PASS`，执行待用户明确批准）**：已从最新 `origin/master` `a6d632ce0c26caa1db9358951eecb40120362107` 创建纯计划分支 `codex/phase1-5-g2-windows-isolation-plan`，计划路径 `.plans/phase1-5-t02-t04-g2-standard-windows-isolation-plan.md`。计划只冻结标准 Windows 一个组合：Windows OS/运行时/CPU 架构记录、既有 Python/FunASR CPU runtime、G1 固定 SenseVoiceSmall/FSMN-VAD 本地模型、任务专属临时 Windows Firewall outbound block rule、无秘密通用出站探测、同一隔离窗口内 16 项安全样例 ASR 矩阵、finally 删除规则、Firewall 规则/进程/临时目录零残留判定和 `PASS`/`ENVIRONMENT_UNAVAILABLE`/`DEFERRED`/`FAIL` 分类。计划明确 Windows Firewall profile 全禁用、权限不足、模型/样例哈希不一致或需要永久系统配置修改时必须停止并记录非 PASS；`offline`/cache-only、baseline、DNS、hosts、代理、人工断网或无 TCP 轮询均不能单独构成 G2 `PASS`。本计划未执行隔离、未创建 Firewall 规则、未运行 ASR、未下载或核验模型、未创建/调用 `AuralConverter`、未修改 `packages/`、Schema、migration、API、Worker、前端或 shared 类型，未启动 T05/T06；下一步执行必须等待用户再次明确批准。
