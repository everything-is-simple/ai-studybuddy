...
**剩余任务数量（2026-07-28 计数口径）**：

| 计数层级 | 数量 | 当前结论 |
| -------- | ---- | -------- |
| Phase 3 顶层候选任务（T01–T05） | **5 项未完成** | T01、T02、T03、T04、T05 均未达到完成标准，当前全部暂停；T02A–T02G 的切片完成不能将顶层 T02 记为完成。 |
| 当前获准在开发机执行的 Phase 3 任务 | **0 项** | 用户已冻结 T02G 边界之后的全部 Phase 3 工作；暂停期间不得把“未完成”理解为“现在应执行”。 |
| T02G 之后的 T02 子任务 | **未定，不能计为固定数量** | T02 总体仍缺真实 Windows ACL、backup/restore、restore 写入、用户电脑实机证据和剩余修复；旧计划中的 Slice 0–5、T02G 计划中的 G1–G5 仅为历史风险/候选分解，尚未成为获批后续任务，不能虚构为确定任务数。 |

> **恢复时的重新计数规则**：只有用户再次明确恢复 Phase 3，并为届时范围创建/批准新的独立计划后，才能把 T02 剩余工作拆成编号任务并更新总数。在此之前，对“Phase 3 还有多少任务要执行”的准确答复是：**5 项顶层候选任务尚未完成，0 项当前获准执行；T02 的后续子任务数未定。** 本次计数澄清仅修改本任务表的状态口径；任务分支已通过 `scripts/check-docs-governance.ps1` 与 `git diff --check`，未运行或修改任何 Phase 3 业务代码、ACL、备份/恢复、部署脚本、服务或会写运行数据的测试。
...
> **2026-07-27 Phase 3 暂停决定**：用户明确要求从 T02G 边界开始暂停开发机上的全部 Phase 3 后续工作。T02G 保留为“仓库外合成夹具范围已完成主线复验”的独立切片事实；该结论不表示真实 Windows ACL、真实 backup/restore、restore 写入、用户电脑实机证据或 T02 总体通过。自本决定起，不再继续拆分或实施任何 T02 后续切片（无论后续编号），不启动 T03/T04/T05，也不创建新的 Phase 3 实施计划。暂停不是完成；未来恢复须由用户再次明确批准，并另建届时适用的独立计划。 本次任务仅同步 `AGENTS.md`、`docs/00`、`docs/04`、`docs/08` 与 T02 总计划的状态措辞；任务分支已通过 `scripts/check-docs-governance.ps1`、`git diff --check` 和暂停边界一致性搜索，未运行或修改 ACL、备份/恢复、部署脚本、业务代码、服务或会写运行数据的测试。
