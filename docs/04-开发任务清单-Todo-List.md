...
---
## Phase 3：打磨与安全（自 T02G 边界起暂停）

**状态**：用户曾于 2026-07-25 批准 T02 安全与隐私基线审计计划；T02A–T02G 已各自完成主线复验并进入 `origin/master`，其中 T02G 仅完成仓库外合成夹具的安全边界实现与 P1 修复。用户于 2026-07-27 明确要求到此为止：T02 总体转为暂停，开发机上的全部 Phase 3 后续工作冻结在 T02G 边界。总体审计、真实机器操作与其余 Phase 3 候选任务仍未完成。

**暂停门禁**：不再继续拆分或实施任何 T02 后续切片，也不启动 T03、T04、T05 或其他开发机 Phase 3 工作；不得把既有计划、单个切片、合成夹具证据或分支提交外推为 T02、Phase 3、真实机器操作、生产上线或用户电脑验收完成。未来如需恢复，必须由用户再次明确批准，并从届时最新 `master` 创建新的独立计划/任务分支。当前继续保持“家长不登录、无公网入口、无家长 Web 面板”的边界；不得默认把历史候选的家长面板作为既定需求。

| 顺序 | 候选任务 | 状态 | 单一责任 |
| ---- | -------- | ---- | -------- |
| 1 | T01：S6 后续产品形态重新决策（历史候选：家长面板） | ⏸️ | 先决定是否继续保持异步脱敏报告；不得默认引入家长登录或 Web 面板 |
| 2 | T02：安全与隐私基线审计 | ⏸️ | T02A–T02G 切片均已推送 `origin/master` 并完成各自主线复验；T02G 仅完成仓库外合成夹具安全边界实现与 P1 修复；总体审计、真实 ACL/backup/restore 与全部修复仍未完成，已按用户 2026-07-27 决定暂停 |
...
**先给结论**：Phase 3 有 **5 个大任务、28 个已定义小任务**。其中 **25 个是完成 Phase 3 必须完成的小任务，3 个是条件触发小任务**。25 个必做小任务中，T02A–T02G 共 **7 个已按各自批准范围完成**，还有 **18 个必做小任务未完成并暂停**。当前用户批准在开发机继续执行的 Phase 3 任务为 **0 个**。
...
- **“必做 25 个”**表示如果将来要正式宣布 Phase 3 完成，这 25 个结果必须全部达到验收标准；不表示现在应该继续做。
- **“条件触发 3 个”**分别是：T01 产品边界改变后才实施变更、发现 P0 后才启动应急处置、当前 MVP 真正启用 Worker 或基线证明需要时才做 Worker 并发优化。
- **单独回答“T02G 后面还有多少”**：T02 还剩 **6 个必做完成项（T02-R1～T02-R6）**，另有 **1 个仅在发现 P0 时触发的应急项**；当前获准执行为 0。`R` 只表示 Todo/WBS 中的 remaining item，不是新实施切片、计划或批准。
...
| T02 | T02-R1 正式仓库与实际部署包秘密扫描签收 | 对届时正式仓库/实际候选部署包执行受控扫描，复核命中并形成不泄露秘密值的签收证据 | **必做** | ⏸️ T02 未签收；Alpha-D0 有可复用部署包扫描证据，但不能自动外推为本项完成 | T02 总计划 §6.1、Slice 2 和 §8；T02D 计划的“合成夹具不等于真实扫描”边界 |
| T02 | T02-R2 真实 Windows 目录与 ACL 证据 | 在批准机器只读取证据：config/data/logs/backups/tmp/models 的实际 ACL、继承、主体和越权风险；发现 P1 时修复并回归 | **必做** | ⏸️ 未完成，暂停 | T02 总计划数据/部署范围、Slice 3、§8；T02G 计划 G2/G5 与“真实操作另批”门禁 |
| T02 | T02-R3 真实备份/恢复安全复核 | 在 T04 能力完成后，复核真实 backup/restore 的受控根、白名单、manifest/hash、服务停止、recovery point、写入顺序、中断处理和无秘密/路径泄露 | **必做** | ⏸️ 未完成，暂停；依赖 T04 | T02 总计划数据边界、Slice 3、§8；T02G 计划 G3/G4；`docs/13-部署运维指南-Deployment.md`“备份、恢复、升级与回滚” |
| T02 | T02-R4 学生核心流程失败反馈矩阵 | S1/S2/S3/S4/S5/S7 核心路径覆盖输入错误、不存在、未配置、超时/下游失败、未知异常与重试建议，前端只显示可操作的脱敏中文信息 | **必做** | ⏸️ 未完成，暂停 | T02 总计划 Slice 4 与 §8 第 7 项 |
...
| T02 | T02-R6 T02 总体回归与完成验收 | 确认 P0=0、全部 P1 修复、自动化回归和 Windows 原生证据通过，文档/风险签收一致后，才能把顶层 T02 标为完成 | **必做** | ⏸️ 未完成，暂停 | T02 总计划 §8“实施验收标准”和 §9 验证集 |
...
| T04 | T04-1 自动定期备份 | 实现可配置周期、受控目标根、白名单 payload、manifest/hash、失败可见且不覆盖未知备份的自动备份 | **必做** | ⏸️ 未完成，暂停；T02G 只有合成安全底座 | 本文件 T04“自动定期备份”；`docs/13` 备份目录与当前未完成边界；T02G 计划 G3 |
| T04 | T04-2 损坏与完整性检测 | 对全局库、学期库和备份执行完整性/hash/manifest 校验，能识别损坏、缺失、篡改和不可恢复状态 | **必做** | ⏸️ 未完成，暂停 | 本文件 T04“损坏检测”；`docs/08-共同底座架构-Architecture.md` 分学期隔离/完整性/恢复要求；T02G 计划 G3/G4 |
| T04 | T04-3 一键恢复与真实恢复演练 | 实现受控恢复写入，包含服务/计划任务停止、恢复点、预检、复制顺序、中断恢复、结果校验和回滚；在批准机器完成真实演练 | **必做** | ⏸️ 未完成，暂停；当前非 `-WhatIf` 固定拒绝 | 本文件 T04“一键恢复验证”；`docs/13`“备份、恢复、升级与回滚”；T02G 计划 G4 |
| T05 | T05-1 全链路日志脱敏 | 覆盖后端、Adapter/Worker、部署脚本和渠道失败日志，禁止秘密、学生原文、完整 UUID、绝对路径和外部原始响应 | **必做** | ⏸️ T02F 已有安全底座，T05 全链路验收未完成 | 本文件 T05“脱敏日志”；T02 总计划“日志与隐私”；`docs/08`/`docs/10-后端开发规范-Backend-Guidelines.md` 日志禁止字段 |
| T05 | T05-2 日志等级、事件与允许字段规范 | 统一 debug/info/warn/error 的使用、事件名、关联 ID、允许字段和面向用户/内部诊断边界 | **必做** | ⏸️ T02F 已有部分契约，T05 整体未完成 | 本文件 T05“分级输出”；T02F 计划 §4；`docs/10` 日志规范 |
| T05 | T05-3 日志轮转与容量上限 | 为运行日志设定单文件/总量上限和安全轮转，证明重复启动与长期运行不会无限增长 | **必做** | ⏸️ T02F 仅完成合成边界，真实长期运行验收未完成 | 本文件 T05“日志轮转”；T02 总计划 Slice 3；T02F 计划 |
