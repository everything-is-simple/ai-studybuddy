...
| 5 | T05：冲刺计划生成 | 🟡 | 实施分支已完成确定性即时只读 API/独立页面及全量验证；待按独立流程合入 `master`。不含 T06 工作台冲刺区 |
...
| T05 | `.plans/phase2-t05-s5-cram-plan-plan.md` | 用户已明确批准实施；实施分支 `codex/phase2-t05-s5-cram-plan` 已基于计划提交 `44c28443c61eeae356031ba9864be51f0f9043f6` 完成确定性即时只读的未来 7 天冲刺计划 API/DTO/Service、独立页面与考试上下文导航。已通过类型检查、两项生产构建、全量测试（frontend 131/131、backend 237/237）及本地 Chromium 3/3；待按独立流程合入 `master`。不实现 T06、S7、S3 Worker、AI/Provider、持久化计划或外部 smoke。 |
| T06 | 尚未创建 | 等待独立计划、独立审查并获用户明确批准；考试工作台“冲刺”区集成未启动。 |
