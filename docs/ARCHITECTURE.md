...
### 2.4 两层架构：格式转换层 vs LLM 理解层（核心边界，已确认）

> **设计原则**：格式转换不是 AI，LLM 只做理解和推理。两层解耦后，DeepSeek 等纯文本模型完全够用，不需要多模态模型。

#### 第一层：格式转换层（不需要 LLM，专用开源工具）

将各种格式的原始素材统一转为纯文本，是管道，不思考，只搬运。
...
- 选择题/填空题批改 → 精确匹配
- 艾宾浩斯复习排程 → 日期计算
- 超时任务检测 → 定时扫描 deadline
- 考前预警触发 → 日期倒推
- 思维导图渲染 → 前端 Markmap.js
...
CREATE TABLE ai_provider_configs (
    ...
    -- 预算控制字段（见 backend-guidelines.md 6.5）
...
| 预算与额度 | 试用额度剩余次数/自备 Key 月费用进度条，超限告警提示（详细规则见 backend-guidelines.md 6.5） |
| 重置默认 | 一键重置为系统推荐的默认配置 |
| 预设方案 | 提供几套预设：省钱方案（全部V4-Flash）、均衡方案、质量方案 |

### 6.6 工具与模型分层总表（2026年7月调研）

> 本节与 2.4 两层架构对齐。所有“格式转换”用专用开源工具，不花 LLM 钱；LLM 调用只负责理解和推理。
...
|------|----------|
| 选择/填空题批改 | 字符串精确匹配 |
| 艾宾浩斯复习排程 | 日期计算（1d/2d/4d/7d/15d/30d） |
| 超时任务检测 | Cron 扫描 deadline |
| 考前预警触发 | 日期倒推 |
| 思维导图渲染 | 前端 Markmap.js，纯渲染 |
| 错题入库 | AI 批改后自动写入，无额外 LLM 调用 |

**成本估算**：一个学生每天 3 节课，每月约 60 次素材转写（免费） + 60 次笔记整理（含导图+高亮，合并为 1 次调用） + 30 次练习，估算月费约 ¥2-5。对比 GPT-4o 同样用量约 ¥150-400。
...
```
输入：任意格式的原始素材（音频/图片/PDF/视频/链接/文本）
  │
  ├─ 音频：
  │     ├─ 下载音频到临时目录
  │     ├─ 音频预处理：降噪、采样率统一到16kHz
  │     ├─ SenseVoice-Small / FunASR 转写
  │     ├─ 输出带时间戳的 segments: [{start, end, text},...]
  │     └─ 后处理：合并短segment、修正标点
  │
  ├─ 图片：
...
输出：统一纯文本 → 写入 transcripts 表或直接进入第二层
```

### 7.3 结构化笔记 Pipeline（第二层，一次 LLM 调用输出三样）

```
输入：纯文本（来自 7.2 的任意输出） + 课程上下文
  │
  ├─ 1. 构造 prompt：
  ...
  │
  ├─ 2. 要求一次调用同时输出三样：
  │     ├─ A. Markdown 章节结构（一、二、三...） + 知识点标注：[定义]、[公式]、[重点]、[例子]
  │     ├─ B. Mermaid 思维导图（mindmap 语法，知识点层级）
  │     └─ C. 高亮列表：[{text:"...", type:"definition|formula|key_point"},...]
  │
  ├─ 3. 解析响应，分别写入：
  │     ├─ structured_notes 表（含 highlights JSONB 字段）
  │     └─ mind_maps 表（Mermaid 文本）
  │
  └─ 4. 末尾附“本课次要点总结”
输出：结构化笔记 + 思维导图 + 重点高亮（全部一次完成）
```
