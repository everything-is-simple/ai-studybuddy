...
- 保持现有 10 个路由页（9 个学习业务页 + 1 个设置页）的职责边界，不为追求页面数量重复拆分笔记、薄弱点或练习历史。
- S6 在当前阶段继续使用邮件和飞书异步报告，不新增家长登录、家长 Web 面板或公网入口。
- Phase 1-T07 已将当前课程最近 8 条学习时间线活动嵌入考试工作台；T08 已实现 `/settings` 本机配置中心。当前下一门禁为 T09A 学期创建、选择与切换。
...
| 口径 | 学生端图形界面数 | 说明 |
| ---- | ---------------- | ---- |
| 当前代码已有 | **10 个路由页 + 1 个应用壳** | 9 个学习业务页 + `/settings` 配置中心 |
| MVP 学习闭环最少 | **8–9 个** | 与当前实现基本重合；S6 不需要学生/家长 Web 页 |
| 带设置入口的当前形态 | **9 个业务页 + 1 个设置入口** | T08 已实施，敏感配置由后端 DPAPI 加密存储 |
...
| 10 | `/settings` | 运行状态、AI Provider、QQ SMTP、飞书 Webhook 配置与连接测试 | 共同底座 |

主导航目前只有「课程与考试 / 资料」两项；练习与错题从**考试工作台**进入。

**已实现学生端图形界面 = 10 个路由页（+ 1 壳）。**
...
当前仓库已有 9 个学习业务路由和 1 个 `/settings` 设置页。若强行把「练习发起」嵌进工作台且不单独路由，学习业务路由可算 **8**，但实现与验收成本更高，不推荐再砍。
...
**排期口径建议：**
- 如果目标是“功能核心能运行”，当前 10 页已经覆盖 S1–S4 和配置中心。
- 如果目标是“学生端可交付 MVP”，仍需 T09A–T09D 消除手输 UUID、补每日首页并统一整条学生旅程。
...
在现有 10 页基础上，PRD 级完整度通常还要：

1. 学期开始向导
2. 每日学习首页（可替代裸 `/courses` 默认首页）
3. 练习历史（可选，可嵌工作台）

T07 已把近期学习时间线嵌入考试工作台，不再把独立时间线路由计为当前缺口。补齐上述候选后约 **13 个独立路由**，若课表、归档再拆页则到 **14–16**。
...
> **当前已有 10 个本机 Web 路由，但需完成 T09A–T09D 才称为学生端可交付 MVP；家长端仍为 0 个 Web 界面。**
...
| L3 当前设置形态 | 9 + 1 个设置入口 | T08 已实现 |
| L4 PRD 完整学生 MVP | 14–16 | L3 + 向导/首页/历史等；近期时间线已嵌入工作台 |
...
| 端 | 最少图形界面 | 说明 |
| -- | ------------ | ---- |
| **前端（当前学生 Web）** | **10** | 已实现 9 个学习业务路由页和 1 个设置页 |
...
- [x] **学期 ID 壳下一步**：T09A 已登记为下一门禁，用学期向导/选择替换手输 UUID；具体路由由 T09A 计划决定。
...
2. Phase 1-T08 已完成：`/settings` 提供运行状态和三通道配置/连接测试，首次配置提示从应用壳进入。
3. 当前进入 T09A 学期创建、选择与切换门禁；T09B 每日首页、T09C 课程/课表/考试完善、T09D 全局学生旅程 E2E、T09E 历史/归档均已登记但未实施。
