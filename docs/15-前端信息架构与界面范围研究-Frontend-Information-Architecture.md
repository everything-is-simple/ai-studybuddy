...
- 保持现有 9 个路由页的职责边界，不为追求页面数量重复拆分笔记、薄弱点或练习历史。
- S6 在当前阶段继续使用邮件和飞书异步报告，不新增家长登录、家长 Web 面板或公网入口。
- Phase 1-T07 已将当前课程最近 8 条学习时间线活动嵌入考试工作台，没有新增独立路由；当前下一门禁为 T08 本机配置中心与连接验收的独立计划、审查和用户明确批准。
- 正式产品应逐步消除“手工输入学期 UUID”的开发期交互，改为可理解的学期创建、选择和切换流程。

### 需要独立门禁后再吸收的候选项

- 每日学习首页：优先考虑与 S1 时间线、今日任务和临近考试整合，而不是单独增加一个只展示统计的首页。
- 练习历史：优先嵌入考试工作台或练习区；只有筛选、分页和跨考试检索复杂度足够高时才独立成页。
- 系统设置入口：可收纳非敏感偏好、运行状态和集成配置状态，但必须先完成设置模型、权限、脱敏与持久化设计。
- 配置中心不是普通偏好页，而是系统首次可用前的本机设置向导：AI Provider、QQ SMTP、飞书 Webhook 必须能够分别保存、测试和显示健康状态。
- 前端视觉升级：应基于现有路由渐进实施，遵循 `docs/11-前端开发规范-Frontend-Guidelines.md`，并单独计划浏览器验收。

### 不吸收的做法

- 不把“10 个页面”或“14–16 个页面”作为里程碑验收指标；验收应以用户闭环和任务门禁为准。
- 不在浏览器、前端日志或 `localStorage` 保存 API Key、SMTP 密码、飞书 Webhook 等秘密。
- 不因为需要配置能力就立即新增后端管理台、家长面板或 Phase 3 功能。
- 不提前实现 S5、S7 或尚未批准的系统配置功能。
...
- 运维分工：可变配置未来可以收敛到统一设置入口，但敏感凭据必须由后端安全存储与脱敏展示；当前仍以环境变量为准，不把秘密直接交给浏览器保存。
- 问题拆成三档回答：
  1. **已实现**有几个学生端界面
...
| 口径 | 学生端图形界面数 | 说明 |
| ---- | ---------------- | ---- |
| 当前代码已有 | **9 个路由页 + 1 个应用壳** | 已覆盖 S1–S4 最小闭环 |
| MVP 学习闭环最少 | **8–9 个** | 与当前实现基本重合；S6 不需要学生/家长 Web 页 |
| 带设置入口的候选形态 | **9 个业务页 + 1 个设置入口** | 是否实施及敏感配置如何存储，需独立计划和安全设计 |
| 按 PRD 完整理想 MVP | **约 14–16 个** | 补学期向导、每日首页、时间线、练习历史等 |
| 家长端 Web 面板 | **0（MVP）** | Phase 3 才考虑；当前只发报告 |

后端本身**不需要**独立图形运维台。统一设置入口是候选产品能力，不是当前既定门禁；邮件 HTML 报告不算产品“界面页”。
...
| # | 路由 | 页面职责 | 子系统 |
| - | ---- | -------- | ------ |
| 壳 | App Shell | 学期 ID 输入、全局标题 | 底座 / 开发期上下文 |
| 1 | `/courses` | 课程列表、创建课程/考试、考试确认入口 | S1 |
| 2 | `/materials` | 资料上传、处理状态、人工补文 | S2 |
| 3 | `/notes/:noteId` | 笔记 Markdown/KaTeX、导图、知识模块 | S2 |
| 4 | `/exams/:examId` | 单考试工作台（倒计时、任务、近期活动、练习/查漏入口） | S1 枢纽 |
| 5 | `/exams/:examId/practice` | 练习发起 | S3 |
| 6 | `/practice-sessions/:sessionId` | 限时作答 | S3 |
| 7 | `/practice-sessions/:sessionId/result` | 批改结果 | S3 |
| 8 | `/exams/:examId/mistakes` | 错题列表 + 薄弱点区块 | S4 |
| 9 | `/mistakes/:mistakeId` | 错题详情、错因确认、原题重做 | S4 |

主导航目前只有「课程与考试 / 资料」两项；练习与错题从**考试工作台**进入。

**已实现学生端图形界面 = 9 个路由页（+ 1 壳）。**
...
| 候选 | 系统配置中心/首次启动向导 | 暂不计入当前闭环 | AI、SMTP、飞书分别测试；普通设置与秘密分离；敏感凭据只由后端保存和使用 |

**结论 A — MVP 学习闭环最少学生界面：8–9 个。**
当前仓库已基本达到该下限（9 路由）。若强行把「练习发起」嵌进工作台且不单独路由，可算 **8**，但实现与验收成本更高，不推荐再砍。

**结论 B — 统一配置中心应登记为后续独立任务；它的完成标准不是“多一个页面”，而是配置保存、后端安全存储、连接测试、状态门禁和失败降级全部可验收。**
...
首次启动可以进入配置中心，后端不因缺少 AI Key、SMTP 或飞书配置而无法启动。配置中心显示四种状态：未配置、已配置未验证、验证通过、验证失败。
...
| 学期 ID 条 | 开发期壳能力；正式产品应被「学期向导/选择」替代，不另算业务页时可不单独计 |
...
| Phase 1 | 练习发起、练习作答、练习结果、错题列表、错题详情/改错 | 9 | 在 Phase 0.8 基础上补齐“练、判、改”闭环。 |
| Phase 1.5 | 录音上传/转写页 | 10 | 只在 S7 触发时加入，面向课堂录音输入链路。 |
| Phase 2 | 模拟考、临考速背、冲刺计划 | 12-13 | 进入考试冲刺后补足高频复习与模拟训练入口。 |
| Phase 3 | 设置与安全能力；家长面板仍需重新决策 | 以获批计划为准 | 设置入口、安全存储和家长面板不是同一任务，不应打包实施。 |

**排期口径建议：**
- 如果目标是“先能用”，先做到 Phase 0.8 + Phase 1，累计 9 页即可跑通学生闭环。
- 如果目标是“能维护”，应先设计设置模型和秘密存储边界，再决定是否前置统一设置入口。
...
| ------ | ------------ | -------- | ---- |
| S1 | 学期开始向导 | ❌ 仅手输 semester UUID | 大缺口 |
| S1 | 每日学习首页 | ❌ | 大缺口 |
| S1 | 课程与课表 | 部分（`/courses` 无完整课表） | 中 |
| S1 | 考试目标 | 部分（列表+确认） | 小–中 |
...
在现有 9 页基础上，PRD 级完整度通常还要：

1. 学期开始向导
2. 每日学习首页（可替代裸 `/courses` 默认首页）
3. 练习历史（可选，可嵌工作台）

T07 已把近期学习时间线嵌入考试工作台，不再把独立时间线路由计为当前缺口。补齐上述候选后约 **12 个独立路由**，若课表、归档再拆页则到 **13–15**。
...
> **前后端合在一起：学生端最少 9 个图形界面即可跑通 MVP 闭环；家长端 MVP 为 0 个 Web 界面；后端 0 个业务 GUI。**
...
| L3 候选设置形态 | 9 + 1 个设置入口 | 需独立计划；不以页数作为验收目标 |
| L4 PRD 完整学生 MVP | 13–15 | L3 + 向导/首页/历史等；近期时间线已嵌入工作台 |
...
| 端 | 最少图形界面 | 说明 |
| -- | ------------ | ---- |
| **前端（当前学生 Web）** | **9** | 已实现 9 个业务路由页；设置入口尚未批准 |
| **后端** | **0** | 无管理后台需求；健康检查可用 curl/日志 |
| **家长渠道** | **0 Web 页** | 邮件/飞书；HTML 报告模板 ≠ 产品页面 |
| **运维/本机维护者** | **0 独立后台** | 当前使用环境变量和本机脚本；未来设置入口需独立设计 |
...
- [ ] **默认首页**：正式产品应以「每日首页」还是继续以 `/courses` 为默认？
- [ ] **学期 ID 壳**：何时用「学期向导」替换手输 UUID？是否算新增第 11 页？
- [ ] **练习历史**：是否必须独立路由，还是工作台列表足够？
...
1. Phase 1-T07 已完成：学习时间线以考试工作台“近期学习活动”落地，不新增独立时间线路由或每日首页。
2. 当前按 `docs/04-开发任务清单-Todo-List.md` 进入 T08 本机配置中心与连接验收的独立计划、审查和用户明确批准门禁。
3. 学期向导、每日首页和练习历史仍是候选任务，不因 T07 完成而自动实施；配置中心的连接测试属于系统可用性门禁，不等同于普通 UI 美化。
4. 不为 S6 单独创建家长 Web 面板，除非未来明确批准 Phase 3 范围。
