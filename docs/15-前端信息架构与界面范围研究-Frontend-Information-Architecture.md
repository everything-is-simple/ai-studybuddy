...
- 保持现有 20 个路由页（19 个学习业务页 + 1 个设置页）的职责边界，不为追求页面数量重复拆分笔记、薄弱点或练习历史。
- S6 在当前阶段继续使用邮件和飞书异步报告，不新增家长登录、家长 Web 面板或公网入口。
- Phase 1-T07/T08/T09A–T09E 已完成时间线、配置中心和学生端产品化；Phase 2-T03–T06 已完成模拟考、临考速背、冲刺计划和考试工作台冲刺区。POST-PHASE2 全系统验证、完整 E2E、文档收口与主线复验已完成并推送 `origin/master`，仍不进入 Phase 3。
...
- 不因本研究文档继续扩展已完成的 S5 MVP、完整 S7 或任何尚未批准的学生端页面；当前获批的 S7-MVP 仅在既有资料页内增加一个受控 WAV 转写卡。
...
| 口径                   | 学生端图形界面数               | 说明                                                            |
| ---------------------- | ------------------------------ | --------------------------------------------------------------- |
| 当前代码已有           | **20 个路由页 + 1 个应用壳**   | 19 个学习业务页 + `/settings` 配置中心                          |
| MVP 学习闭环最少       | **8–9 个**                     | 与当前实现基本重合；S6 不需要学生/家长 Web 页                   |
| 带设置入口的当前形态   | **19 个业务页 + 1 个设置入口** | Phase 1 学生端与 Phase 2 S5 均已集成                            |
| Phase 1 学生端 MVP     | **14 个**                      | T09A–T09E 已补齐学期、首页、导航、练习历史与归档                |
| Phase 2 学生端当前形态 | **20 个**                      | 新增模拟考 4 页、临考速背 1 页、冲刺计划 1 页；冲刺区嵌入工作台 |
| 家长端 Web 面板        | **0**                          | 当前只发报告；Phase 3 已明确暂缓，是否变更该边界需重新决策      |
...
共同底座（无独立业务 UI）
  ...
  + S5 期末冲刺  → Phase 2 已实现，新增模拟考/速背/计划路由并集成工作台
  + S7-MVP 课堂录音转文字 → 已完成主线复验并推送 `origin/master`：复用资料页卡片，不新增主要路由
```

信息架构原则（`docs/04` Phase 1）：页面围绕**一个考试项目工作台**组织：

```text
总览 → 资料 → 计划 → 练习 → 查漏补缺 → 冲刺（模拟考 / 速背 / 每日计划）
```
...
| 14  | `/exams/:examId/mock-exam`                           | 当前考试模拟考生成入口                                      | S5                 |
| 15  | `/mock-exam-papers/:paperId`                         | 模拟卷详情与开始作答                                        | S5                 |
| 16  | `/mock-exam-attempts/:attemptId`                     | 限时模拟考作答与刷新恢复                                    | S5                 |
| 17  | `/mock-exam-attempts/:attemptId/result`              | 模拟考成绩、逐题结果与模块分析                              | S5                 |
| 18  | `/exams/:examId/cram`                                | 临考速背卡与总倒计时                                        | S5                 |
| 19  | `/exams/:examId/cram-plan`                           | 未来 7 天确定性冲刺建议                                     | S5                 |
| 20  | `/settings`                                          | 运行状态、AI Provider、QQ SMTP、飞书 Webhook 配置与连接测试 | 共同底座           |

应用壳提供统一全局导航和当前学期恢复；练习、错题与 S5 冲刺能力均从**考试工作台**进入，练习历史从学期管理进入。

**已实现学生端图形界面 = 20 个路由页（+ 1 壳）。**
...
当前仓库已有 19 个学习业务路由和 1 个 `/settings` 设置页，已经超出上述“最少概念界面”集合。**8–9 个只用于说明闭环最小职责，不是当前路由计数**；现有独立路由经过实现与验收，不因本研究回退或强行合并。
...
| Phase 1.5                         | 资料页内课堂录音转写卡                                      | 已完成主线复验并推送 `origin/master`；不新增主要路由 | 仅受控 WAV、同步本机转写、编辑后保存 S2；不阻塞已完成 Phase 2。     |
| Phase 2                           | 模拟考 4 页、临考速背 1 页、冲刺计划 1 页；冲刺区嵌入工作台 | 20                                                   | 已完成主线集成、复验并推送 `origin/master`。                        |
| Phase 3                           | 暂缓                                                        | 仍为 20                                              | 不新增家长登录或 Web 面板；未来若恢复须重新决策产品边界并独立获批。 |

**排期口径建议：**

- 如果目标是“功能核心能运行”，T08 时点的 10 页已覆盖 S1–S4 和配置中心。
- 如果目标是“学生端可交付 MVP”，Phase 1 的 14 页已完成 T09A–T09E 学期、首页、导航、练习历史与归档闭环。
- 当前 Phase 2 完成后共有 20 个主要页面；S7-MVP 在现有资料页内实施，不增加主要路由；完整 S7 与 Phase 3 均不在当前实施范围。
...
| S5     | 模拟考 / 速背 / 冲刺         | ✅ 6 个独立路由 + 工作台冲刺区                 | Phase 2 MVP 已完成；StudyEvent、S6 扩展和真实 AI 仍未实现 |
| S7     | 资料页内录音转写卡           | ✅ S7-MVP 已完成主线复验并推送 `origin/master` | 不新增主要路由；完整 S7 不计 MVP                          |

### 5.2 完整理想 MVP 学生端粗算

T09A–T09E 已将学期向导、每日首页、完整课程课表/考试目标、全局导航、练习历史与学期归档纳入 Phase 1 的 14 个路由；Phase 2 再增加 6 个 S5 路由。T07 的近期学习时间线和 T06 的冲刺区均嵌入考试工作台，不单独增加路由。

因此当前事实口径是 **20 个独立路由 + 1 个应用壳**；后续新增页面仍须独立门禁，而不是本研究稿自动产生的缺口。
...
> **当前已有 20 个本机 Web 路由，Phase 1 学生端产品化与 Phase 2 S5 已完成；家长端仍为 0 个 Web 界面。Phase 3 仅恢复高权重必做项，不包含家长端页面。**
...
| L5 Phase 2（+S5）   | 20                | 模拟考 4 页、速背 1 页、冲刺计划 1 页，冲刺区嵌入工作台 |
| L6 Phase 3          | 20（暂缓）        | 当前不新增家长登录、报告站或仪表盘                      |
...
| 端                       | 最少图形界面   | 说明                                          |
| ------------------------ | -------------- | --------------------------------------------- |
| **前端（当前学生 Web）** | **20**         | 已实现 19 个学习业务路由页和 1 个设置页       |
...
- [ ] **Phase 3 产品边界**：当前已暂缓并继续坚持家长不登录、无 Web 面板；只有未来明确恢复 Phase 3 时才重新决策。
...
4. Phase 2-T03–T06 已完成 S5 六个新增路由与工作台冲刺区；当前只做 POST-PHASE2 验证收口。
5. Phase 3 仅按高权重必做恢复计划逐项推进；不为 S6 创建家长登录或 Web 面板。S7-MVP 已在既有资料页完成主线复验并推送 `origin/master`；完整 S7 仍等待后续门禁。
...
| 版本 | 日期       | 说明                                                                                                                         |
| ---- | ---------- | ---------------------------------------------------------------------------------------------------------------------------- |
| v0.9 | 2026-07-28 | 同步 Phase 3 仅恢复高权重必做项的当前事实；不新增家长端 Web 界面。                                                           |
| v0.8 | 2026-07-21 | 同步 POST-PHASE2 分支与主线完整验证均已通过并推送；继续保持 20 个路由页、Phase 3 暂缓和家长端无 Web 面板边界。               |
| v0.7 | 2026-07-21 | 对齐 Phase 2 S5 已完成后的 20 个路由页事实，登记模拟考/速背/冲刺计划与工作台集成，并明确 Phase 3 暂缓、家长端仍无 Web 面板。 |
| v0.6 | 2026-07-20 | 同步 Phase 1-T09A–T09E、配置中心与当前 14 路由事实。                                                                         |
