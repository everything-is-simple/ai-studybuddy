...
- 当前下一门禁仍是 Phase 1-T07：S1 时间线扩展；每日入口、时间线和学期上下文应在 T07 及其后续独立计划中逐步收敛。
...
| 4 | `/exams/:examId` | 单考试工作台（倒计时、任务、练习/查漏入口） | S1 枢纽 |
...
3. 学习时间线（可与首页合并为 Tab，但建议独立路由）
4. 练习历史（可选，可嵌工作台）

→ **约 12–13 个独立路由**，若课表、归档再拆页则到 **14–16**。
...
| L4 PRD 完整学生 MVP | 14–16 | L3 + 向导/首页/时间线/历史等 |
...
1. 当前按 `docs/04-开发任务清单-Todo-List.md` 先制定并审批 Phase 1-T07 S1 时间线扩展计划。
2. T07 计划应评估时间线放在考试工作台、每日入口或两者之间共享的方式，不凭本研究稿直接新增路由。
3. 学期向导、每日首页、练习历史和配置中心分别作为候选任务，在触发对应门禁时写独立计划；配置中心的连接测试属于系统可用性门禁，不等同于普通 UI 美化。
