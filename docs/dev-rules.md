...
> 本节规则优先于 3.1–3.3 的所有条目。当 AI 在写代码或做设计决策时，遇到以下情况**必须先停下来提出问题、等待确认**，不得自行决定后继续：

| 触发条件 | AI 必须做的事 |
|----------|---------------|
| 发现设计文档之间有矛盾（如字段命名不一致、表结构描述冲突） | **立即指出矛盾点**，列出涉及的文档和位置，等待人工确认以哪个为准 |
| 发现需求/设计与当前实现有冲突 | **立即指出冲突**，说明影响范围，等待确认后再动手 |
| 觉得某个要求不合理、有更好的方案 | **先提出质疑和备选方案**，等待确认；不得「觉得不对但还是照做」 |
| 对某个技术选型、设计决策有疑问（不确定对不对） | **先问清楚**，得到明确答复后再执行；不得「先改了再说」 |
| 遇到文档中没有覆盖的边界情况，需要自行做设计决策 | **先说明情况和拟采用的方案**，等待确认 |

**核心原则：先问，再做，不能先改再报告。**

- ✅ 正确：「我发现 A 文档说 X，但 B 文档说 Y，两者矛盾，请确认以哪个为准？」→ 等待回答
- ✅ 正确：「这个接口设计我觉得有问题，原因是…，建议改为…，你觉得呢？」→ 等待确认
- ❌ 错误：发现矛盾但没提，直接按自己理解改了
- ❌ 错误：不确定对不对，先改代码，改完再说「我做了一个假设」
- ❌ 错误：觉得需求不合理，但闷头照做，做完才说「我觉得这样不太好」
...
本项目采用“先参考成熟开源，再按本项目架构重写”的策略。任何搬运都必须遵守：

- 不 fork 单一大系统作为总底座。
- 不为参考项目改变本项目主技术栈。
- 优先搬产品流程、数据模型思想、Prompt 思路和交互经验。
- 复制源码前必须确认 License；复制后必须记录来源 URL、License、原作者和修改说明。
- 第三方能力必须通过 Adapter/Provider/Worker 接入，禁止路由层直接调用外部 SDK。
- 家长端隐私边界不可被参考项目覆盖：不展示题目、答案、分数、具体错因。
- AI 输出给学生的是教学解析步骤，不直接暴露模型内部推理全文。
