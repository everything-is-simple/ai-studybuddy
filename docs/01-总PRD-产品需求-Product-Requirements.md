...
```mermaid
flowchart TD
  A["学生创建课程与考试目标"] --> B["上传资料：PDF / 文本 / 图片"]
  B --> C["格式转换：pdf-parse / RapidOCR（无LLM）"]
  C --> D["统一纯文本入库"]
  D --> E["AI整理：中转 GPT/Claude 默认，Kimi 备选，国产官方直连兜底"]
  E --> F["结构化笔记 + 重点 + 思维导图"]
  F --> M["知识模块：重要度、难度、考察内容、来源证据"]
  M --> N["规则生成每日学习任务；逾期任务滚入下一学习日"]
  N --> G["围绕知识模块生成限时练习"]
  G --> H1["客观题规则批改"]
  G --> H2["主观题 LLM 辅助评分"]
  H1 --> I["错题入库"]
  H2 --> I
  I --> W["归纳薄弱点并提升相关模块/任务优先级"]
  W --> J["错题复习任务进入学习节奏"]
  J --> K["22:30 向家长发送脱敏学习趋势与考前提醒"]
```
...
KnowledgeModule → StudyTask → StudyEvent
KnowledgeModule → Question → PracticeSession / PracticeAnswer → Mistake → WeakPoint
Exam + Task + StudyEvent → ParentReport（只做脱敏聚合）
```

- `Exam` 记录考试名称、日期、目标、每日可学习时间和范围；它驱动计划、模拟考和考前 7/3/1 天提醒。
...
- 家长报告只读取课程名、考试临近状态、任务/学习事件和趋势；不读取资料正文、笔记正文、答案或错题内容。
