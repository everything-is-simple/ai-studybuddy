...
**状态**：已触发并生效
**日期**：2026-07-23
**适用范围**：当前 Windows 原生单机部署准备；不包含 S7 课堂录音正式产品装配、真实 ASR Worker/API/前端接入、公网入口、云端数据库或 Docker/WSL 常驻产品依赖。
...
本轮 v1 部署采用“受控前置运行时”：使用机器需要先具备经过验证的 Node.js 和 x64 Python；部署脚本在用户目录创建 Python venv 并安装锁定 OCR 依赖。真正无 Node/Python 前置依赖的 MSI/便携版不在本轮范围内。
...
Node native 依赖（例如 `better-sqlite3`）需要匹配当前 Node 的预编译包；若预编译下载失败，npm 可能退回本地编译并要求 Visual Studio C++ Build Tools。bootstrap 已重试 npm 安装，但使用机器仍应使用兼容清单中记录的受验证 Node/Python 组合，并保持网络可访问生产依赖源。
...
实际恢复前必须停止后端和计划任务。实际恢复默认先在 `backups\recovery-<timestamp>` 保留恢复点，再复制备份 payload。升级流程固定：停止后端和计划任务 → 生成只读备份 → 复制新应用包 → 运行 migration/健康检查 → 验收通过后切换；失败时保留旧应用包和旧数据库，不覆盖回滚源。
...
本轮不包含：S7 课堂录音正式产品装配、`AuralConverter`、真实 ASR Worker/API/前端接入、真实用户音频、公网入口、家长远程登录、云端数据库、Docker 作为正式产品依赖、修改全局 Windows 防火墙策略、无备份/无确认迁移真实学习数据。
