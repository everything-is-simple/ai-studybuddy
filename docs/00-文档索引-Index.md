...
2. Phase 1-T00、T10、T02、T03、T11、T03A、T03B、T03C、T03D、T04、T04A、T04B、T05、T07 与 T08 已完成；T09A 已在任务分支完成实现与验证、等待固定合入和 master 复验；S3 已完成 Schema、练习生成/提交批改 API、前端发起/作答/结果闭环，S4 已完成错题归档、错题改错前端与回流规则，T07 已在考试工作台展示当前课程近期时间线活动，T08 已提供本机配置中心与连接验收；
...
4. 当前门禁为 T09A 的固定 fast-forward 合入 `master` 与 master 复验；在 `origin/master` 包含提交前，T09A 不得报告完成，也不得启动 T09B–T09E。S5/S7 继续按各自门禁等待。S3 Worker 不属于当前 MVP，仍未开始。
5. `docs/04` 的“Phase 1 行动计划索引”是任务与 `.plans/` 的对应 SoT；T09A 的 v4 计划已获用户明确批准，任务分支实现与验证完成但尚未合入/推送 `origin/master`；T09B–T09E 仍仅登记路线、计划尚未创建。
...
| 版本 | 日期 | 变更 |
| ---- | ---- | ---- |
| v2.27 | 2026-07-18 | T09A 用户批准后完成任务分支实现和隔离验证：正式学期创建/选择/切换/current 恢复、migration v8、跨学期隔离与移除手输 UUID；实现提交 `965ee4f` 已创建，文档证据随任务分支提交登记，待任务分支推送、合入 master 与 master 复验，T09B–T09E 未启动 |
