...
test('T12 设置中心通过 API mock 提供预设、脱敏失败与窄屏交互', async ({ page }) => {
  ...
  await page.route('**/api/config/**', async (route) => {
    ...
    if (request.method() === 'POST' && pathname === '/api/config/ai/test-and-activate') {
      ...
      if (aiActivationCount > 1) {
        await route.fulfill({
          status: 422,
          json: { success: false, error: { code: 'CONFIG_CONNECTION_TEST_FAILED', message: '连接测试失败' } },
        });
        return;
      }
    ...
    await route.fulfill({
      status: 500,
      json: { success: false, error: { code: 'UNEXPECTED_REQUEST', message: '未预期请求' } },
    });
  });
  ...
  const relayCard = page.locator('.provider-preset-card', {
    has: page.getByRole('heading', { name: '中转站 1', level: 4 }),
  });
  ...
  const kimiCard = page.locator('.provider-preset-card', {
    has: page.getByRole('heading', { name: 'Kimi / Moonshot', level: 4 }),
  });
  ...
  expect(receivedAiPayload).toEqual({
    providers: [
      { kind: 'official', presetId: 'kimi', apiKey: 'FAKE_E2E_KIMI_KEY', model: 'kimi-k2.7-code', priority: 1 },
    ],
  });
  ...
  currentStatus = {
    ...
    smtp: {
      ...
      summary: 'QQ SMTP 已激活',
      details: [
        { label: '账号', value: 'se••••••@example.test' },
    ...
    feishu: {
      status: 'environment_fallback',
      lastVerified: null,
      summary: '飞书 Webhook 已激活',
      details: [{ label: '飞书 Webhook', value: '•••••••• 已保存，不可回显' }],
      errorCode: null,
    },
  };
  await page.reload();
