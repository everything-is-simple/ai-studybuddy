...
const cards = [
  { id: 'synthetic-card-1', knowledgeModuleId: 'synthetic-card-1', title: '函数概念', importance: 'critical', contentSummary: '合成摘要：先确认输入与输出关系。', examRelevance: '合成考点：辨析函数定义。', sources: [{ kind: 'weak_point', count: 3 }, { kind: 'knowledge_module', count: 1 }] },
  { id: 'synthetic-card-2', knowledgeModuleId: 'synthetic-card-2', title: '方程关系', importance: 'high', contentSummary: '合成摘要：先整理等式条件。', examRelevance: '合成考点：检查解的条件。', sources: [{ kind: 'mistake', count: 2 }, { kind: 'knowledge_module', count: 1 }] },
];
...
async function installCramApi(page: Page): Promise<void> {
  await page.route('http://127.0.0.1:4311/api/**', async (route) => {
    ...
    if (request.method() === 'GET' && path === '/api/semesters/current') {
      return json(success({ semester: { id: semesterId, semesterCode: 'E2E', studentName: '合成学生', teachingStartDate: '2026-02-16', teachingEndDate: '2026-06-30', finalArchiveDate: null, status: 'active', isCurrent: true, createdAt: '2026-07-21T00:00:00.000Z', updatedAt: '2026-07-21T00:00:00.000Z' }, recoveredFromStaleCurrent: false }));
    }
    if (request.method() === 'GET' && path === `/api/exams/${examId}`) {
      return json(success({ id: examId, courseInstanceId: courseId, name: '合成临考考试', attemptType: 'normal', examAt: '2026-08-01T09:00:00.000Z', confirmationStatus: 'confirmed' }));
    }
...
test('T04 临考速背在 Chrome 中支持翻卡、刷新恢复、超时锁定与窄屏', async ({ page }) => {
  ...
  const stored = await page.evaluate(() => sessionStorage.getItem('ai-studybuddy:cram:synthetic-semester:synthetic-exam'));
