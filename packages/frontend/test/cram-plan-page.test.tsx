  ...
  days: [
    {
      ...
      suggestions: [
        ...
        },
        {
          ...
          priority: 2,
          reason: '薄弱点已有 3 条证据',
          sourceKind: 'weak_point' as const,
          sourceId: 'weak-1',
          targetType: 'weak_point' as const,
          targetId: 'weak-1',
        },
        {
          id: 'mistake-1',
        ...
        },
        {
          ...
          sourceKind: 'practice_performance' as const,
          sourceId: 'practice-1',
          targetType: 'practice_history' as const,
          targetId: 'practice-1',
        },
        {
          id: 'cram-cards',
          ...
          sourceKind: 'cram_cards' as const,
          sourceId: null,
          targetType: 'cram_cards' as const,
          targetId: 'exam-1',
        },
      ],
    },
...
function Switcher() {
  ...
  return (
    <button type="button" onClick={() => navigate('/exams/exam-2/cram-plan')}>
      切换考试
    </button>
  );
}
...
describe('T05 冲刺计划页面', () => {
  ...
  it('renders daily read-only suggestions, navigation and actionable deep links', async () => {
    ...
    const planNav = [...container!.querySelectorAll<HTMLAnchorElement>('a')].find(
      (link) => link.textContent === '冲刺计划'
    );
    expect(planNav?.getAttribute('aria-current')).toBe('page');
    expect(
      [...container!.querySelectorAll<HTMLAnchorElement>('a')]
        .filter((link) => link.textContent === '前往复习')
  ...
  it('renders explicit window states and the safe empty manual-review state', async () => {
    for (const [availability, message] of [
      ['not_started', '尚未进入冲刺窗口'],
      ['ended', '冲刺期已结束'],
      ['available', '暂时没有可安全生成的建议'],
    ] as const) {
  ...
  it('clears stale suggestions immediately while switching examinations', async () => {
    ...
    await act(async () => {
      resolveExam2?.({ ...confirmedExam, id: 'exam-2', name: '另一场考试' });
    });
    getCramPlanMock.mockResolvedValue({
      ...availablePlan,
      assessmentAttemptId: 'exam-2',
      assessmentName: '另一场考试',
    });
    await flush();
