...
export function MockExamResultPage({ semesterId }: MockExamResultPageProps) {
  ...
  const { draft, isHydrated, clearAnswerFields } = useMockExamDraft(semesterId ?? '', attemptId, questionIds, {
  ...
  return (
    <div className="page mock-exam-result-page">
      <Link to={attempt ? `/exams/${encodeURIComponent(attempt.assessmentAttemptId)}/mock-exam` : '/courses'}>
        返回模拟考入口
      </Link>
      <header className="card practice-result-summary">
        <p className="workbench-eyebrow">模拟考结果</p>
        <h1>
      ...
      <section className="practice-result-list" aria-label="逐题批改结果">
        {result.answers.length === 0 ? (
          <FeedbackMessage state="empty" message="结果详情不可用。" />
        ) : (
          result.answers.map((answer, index) => (
            <article className="card" key={answer.questionId}>
              <h2>
                第 {index + 1} 题 · {answer.isCorrect ? '回答正确' : '需要复盘'}
              </h2>
              <p>你的答案：{answer.studentAnswer || '未作答'}</p>
              <p>正确答案：{answer.correctAnswer}</p>
              <p>
                得分：{answer.scoreAwarded} / {answer.pointValue}
              </p>
