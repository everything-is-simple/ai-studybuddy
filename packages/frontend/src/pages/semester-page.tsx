...
import { confirmSemester, listSemesters, previewSemesterTimetable, selectCurrentSemester } from '../api/semester-api';
...
export function SemesterPage({ current, currentMessage = null, onCurrentChange }: SemesterPageProps) {
  ...
  const requiresStudentName = semesters.length === 0;

  useEffect(() => {
    const controller = new AbortController();
    setLoading(true);
    void listSemesters(controller.signal)
      .then((items) => {
        setSemesters(items);
        setListError(null);
      })
      .catch((err) => setListError(err instanceof Error ? err.message : '学期列表加载失败'))
