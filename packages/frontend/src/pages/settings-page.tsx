  ...
  useEffect(() => {
    void Promise.all([getConfigurationStatus(), getConfigurationPresets()])
      .then(([nextStatus, nextPresets]) => {
        ...
        setSmtp((current) => ({
          ...current,
          host: nextPresets.smtp.host,
          port: nextPresets.smtp.port,
          secure: nextPresets.smtp.secure,
        }));
      })
  ...
  function clearSecretFields(channel: ConfigChannel) {
    if (channel === 'ai') {
      ...
      setOfficialDrafts((current) =>
        Object.fromEntries(Object.entries(current).map(([id, draft]) => [id, { ...draft, apiKey: '' }]))
      );
    ...
    <section className="page settings-page">
      <header className="page-header">
        <div>
          <h1>本机配置中心</h1>
          <p>
            配置中心测试激活的连接信息使用当前 Windows 用户的 DPAPI 加密存储；环境 fallback
            来自进程环境或命中的配置文件。页面只显示脱敏摘要，密钥不会回显。
          </p>
        </div>
      </header>
      ...
      >
        ...
        <details className="advanced-config" data-testid="custom-provider-advanced" open>
          ...
          <div className="settings-form">
            <TextInput
              ...
              onChange={(name) => setCustomProvider((current) => ({ ...current, name }))}
            />
            <TextInput
              label="API 地址"
              testId="custom-provider-base-url"
              value={customProvider.baseUrl}
              onChange={(baseUrl) => setCustomProvider((current) => ({ ...current, baseUrl }))}
            />
            <SecretInput
              label="API Key"
              visibilityLabel="自定义 Provider API Key"
              placeholder="已保存的 Key 不会回显；输入新值才替换"
              testId="custom-provider-api-key"
              value={customProvider.apiKey}
              onChange={(apiKey) => setCustomProvider((current) => ({ ...current, apiKey }))}
            />
            <TextInput
              label="模型"
              testId="custom-provider-model"
              value={customProvider.model}
              onChange={(model) => setCustomProvider((current) => ({ ...current, model }))}
            />
          </div>
          <button type="button" className="button-secondary" onClick={addCustomProvider}>
            加入自定义 Provider
          </button>
        </details>
        {providerGroupOrder.map((group) => {
          ...
          return (
            <section className="provider-preset-group" key={group} aria-label={providerGroupLabels[group]}>
              ...
              <div className="provider-preset-grid">
                {providers.map((preset) => (
                  <ProviderPresetCard
                    key={preset.id}
                    preset={preset}
                    draft={officialDrafts[preset.id] ?? blankCardDraft(preset)}
                    onChange={(patch) => updateOfficialDraft(preset, patch)}
                    onAdd={() => addOfficialProvider(preset)}
                  />
                ))}
              </div>
            </section>
        ...
        <FallbackList
          ...
          presets={presets.ai}
          onMove={(from, to) => setFallbackProviders((current) => moveItem(current, from, to))}
          onRemove={(index) =>
            setFallbackProviders((current) => current.filter((_, currentIndex) => currentIndex !== index))
          }
        />
        <div className="settings-actions">
          <button type="button" disabled={busy === 'ai' || fallbackProviders.length === 0} onClick={activateAi}>
            测试并激活 AI
          </button>
        </div>
      </ConfigCard>
      ...
      <ConfigCard
        ...
        onRetest={() => void retest('smtp')}
      >
        <div className="settings-form">
          <TextInput
            label="QQ 邮箱账号"
            placeholder="已有账号请见上方脱敏摘要；输入新值才替换"
            testId="smtp-user"
            value={smtp.user}
            onChange={(user) => setSmtp((current) => ({ ...current, user }))}
          />
          <SecretInput
            label="SMTP 授权码"
            visibilityLabel="SMTP 授权码"
            placeholder="•••••••• 已保存，不可回显；输入新值才替换"
            testId="smtp-auth-code"
            value={smtp.authCode}
            onChange={(authCode) => setSmtp((current) => ({ ...current, authCode }))}
          />
          <TextInput
            label="收件邮箱"
            placeholder="已有邮箱请见上方脱敏摘要；输入新值才替换"
            testId="smtp-to"
            value={smtp.to}
            onChange={(to) => setSmtp((current) => ({ ...current, to }))}
          />
        </div>
        ...
        <details className="advanced-config" data-testid="smtp-advanced">
          ...
          <div className="settings-form">
            <TextInput
              ...
              testId="smtp-host"
              value={smtp.host}
              onChange={(host) => setSmtp((current) => ({ ...current, host }))}
            />
            <label>
              端口
              <input
        ...
        <button type="button" disabled={busy === 'smtp'} onClick={() => void activate('smtp', smtp)}>
          测试并激活 SMTP
        </button>
      </ConfigCard>
      ...
      <ConfigCard
        ...
        onRetest={() => void retest('feishu')}
      >
        <SecretInput
          label="飞书群机器人 Webhook URL"
          visibilityLabel="飞书 Webhook"
          placeholder="•••••••• 已保存，不可回显；输入新值才替换"
          testId="feishu-webhook-url"
          value={feishu.webhookUrl}
          onChange={(webhookUrl) => setFeishu({ webhookUrl })}
        />
        <p className="settings-note">
          {presets.feishu.securityHint || 'Webhook 会加密保存在本机、页面不回显、不要复制到截图或提交到 Git。'}
        </p>
        <button type="button" disabled={busy === 'feishu'} onClick={() => void activate('feishu', feishu)}>
          测试并激活飞书
        </button>
      </ConfigCard>
    </section>
...
function ProviderPresetCard({
  ...
  onChange: (patch: Partial<ProviderCardDraft>) => void;
  onAdd: () => void;
}) {
  ...
  const updateBaseUrl = (index: number, value: string) => {
    invalidateTest();
    onChange({
      baseUrls: baseUrls.map((url, position) => (position === index ? value : url)),
  ...
  const needsManualModel = needsBaseUrl && !testResult?.success && testResult?.error?.includes('/models');

  const availableModels = testResult?.success && testResult.models ? testResult.models : preset.modelSuggestions;

  return (
    <article className="provider-preset-card">
      ...
      {unavailable ? (
        <button type="button" disabled>
          {preset.displayName}（后续适配）
        </button>
      ) : (
        <>
          {needsBaseUrl && (
            <fieldset className="relay-base-urls">
              <legend>API 请求地址（可填多个，逐个尝试）</legend>
              {baseUrls.map((url, index) => (
                <div className="relay-base-url-row" key={index}>
                  <input
                  ...
                  {baseUrls.length > 1 && (
                    <button
                      ...
                      aria-label={`移除${preset.displayName}请求地址 ${index + 1}`}
              ...
              {baseUrls.length < maxBaseUrls && (
                <button
                  type="button"
                  className="button-secondary"
                  data-testid={`official-${preset.id}-add-base-url`}
                  onClick={addBaseUrl}
                >
                  加一个备用地址
                </button>
          ...
          <SecretInput
            label="API Key"
            visibilityLabel={`${preset.displayName} API Key`}
            placeholder="已保存的 Key 不会回显；输入新值才替换"
            testId={`official-${preset.id}-api-key`}
            value={draft.apiKey}
            onChange={(apiKey) => {
              invalidateTest();
              onChange({ apiKey });
          ...
          {testResult?.success && testResult.resolvedBaseUrl && needsBaseUrl && (
            <p className="settings-note">
              实际可用地址：<code>{testResult.resolvedBaseUrl}</code>
            </p>
          )}
          {testResult && !testResult.success && (testResult.attempts?.length ?? 0) > 1 && (
            <ul className="relay-attempt-list">
              {testResult.attempts!.map((attempt) => (
                <li key={attempt.baseUrl}>
                  <code>{attempt.baseUrl}</code> · {attempt.errorCode ?? '失败'}
                </li>
              ))}
            </ul>
          )}
          {needsManualModel && (
            <label className="relay-manual-model">
              模型名（手动填写）
              <input
          ...
          <label>
            模型
            <select
              data-testid={`official-${preset.id}-model`}
              value={draft.model}
          ...
          <button type="button" className="button-secondary" onClick={onAdd} disabled={!testResult?.success}>
            加入 fallback
          </button>
          {!testResult?.success && (
            <p className="settings-note">
              {needsBaseUrl
                ? '请先填写地址和 Key 并测试，测试成功后才能选择模型并加入 fallback'
                : '请先测试 Provider，测试成功后才能选择模型并加入 fallback'}
            </p>
          )}
        </>
      )}
...
function FallbackList({
  ...
  return (
    <section className="fallback-list" aria-label="Provider fallback 优先级">
      <h3>Provider fallback 优先级</h3>
      <ol>
        {providers.map((provider, index) => {
          const label =
            provider.kind === 'official'
              ? (presets.find((preset) => preset.id === provider.presetId)?.displayName ?? provider.presetId)
              : provider.name;
          const host = readProviderHost(provider);
          return (
            <li key={`${provider.kind}-${index}`}>
              <div>
                <strong>{label}</strong>
                <span>
                  {provider.model} · 优先级 {index + 1}
                  {host ? ` · ${host}` : ''}
                </span>
              </div>
  ...
  return (
    <section className="settings-card" aria-labelledby={headingId}>
      <div className="settings-card-head">
        ...
        <span className={`status-badge ${verified ? 'status-success' : environmentFallback ? 'status-warning' : ''}`}>
          {badge}
        </span>
      </div>
      {state?.summary && (
        <p>
          {state.summary} · {state.lastVerified ?? (environmentFallback ? '尚未在本机配置中心验证' : '验证时间未知')}
        </p>
      )}
      {configured && <p className="settings-note">敏感凭据：•••••••• 已保存，不可回显。填写新值并测试激活才会替换。</p>}
      {!!state?.details?.length && (
        <dl className="settings-summary" aria-label={`${title} 已有配置摘要`}>
      ...
      <div className="settings-actions">
        {configured && (
          <button type="button" className="button-secondary" disabled={busy} onClick={onRetest}>
            {environmentFallback ? '测试现有配置' : '重新测试'}
          </button>
        )}
