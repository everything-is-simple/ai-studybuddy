...
export function CramCardsPage({ semesterId, onSemesterError }: CramCardsPageProps) {
  ...
  const currentCard = session.snapshot
    ? (cards.find((card) => card.id === session.snapshot?.currentCardId) ?? null)
    : null;

  if (!semesterId) {
    return (
      <div className="page">
        <FeedbackMessage state="empty" message="请先创建或选择当前学期，才能进行临考速背。" />
      </div>
    );
  }
  ...
  const materialPath = data?.exam.courseInstanceId
    ? `/materials?courseInstanceId=${encodeURIComponent(data.exam.courseInstanceId)}`
    : '/materials';
      ...
      {data && (
        <>
          ...
          {data.exam.confirmationStatus !== 'confirmed' && (
            <FeedbackMessage state="empty" message="请先确认考试信息，确认后才能开始临考速背。" />
          )}
          {data.exam.confirmationStatus === 'confirmed' && cards.length === 0 && (
            <section className="card cram-empty" data-testid="cram-empty">
              ...
              <div className="cram-manual-links">
                <Link className="button-link" to={mistakePath}>
                  查看错题本
                </Link>
                <Link className="button-link" to={materialPath}>
                  查看知识资料
                </Link>
              </div>
            </section>
          ...
          {data.exam.confirmationStatus === 'confirmed' &&
            ...
            !session.snapshot && (
              <section className="card cram-setup" data-testid="cram-setup">
                ...
                <div className="cram-duration-options" role="radiogroup" aria-label="临考速背时长">
                  {([5, 10, 15] as const).map((minutes) => (
                    <button
                      ...
                      className={durationMinutes === minutes ? 'active' : undefined}
                ...
                <button type="button" className="primary-button" onClick={() => session.start(durationMinutes)}>
                  开始速背
                </button>
                {!session.canPersist && (
                  <p className="text-muted">当前浏览器无法保存本次进度；仍可开始，但刷新后会重新开始。</p>
                )}
              </section>
            )}
          {currentCard && session.snapshot && (
            <section className="card cram-session" data-testid="cram-session">
              <header className="cram-session-header">
                <div>
                  <h2>
                    第 {session.currentIndex + 1} / {session.snapshot.cardIds.length} 张
                  </h2>
                  <p>已阅 {session.snapshot.viewedCardIds.length} 张</p>
                </div>
                <div className="cram-timer" aria-live="polite">
                  <strong>{formatRemaining(session.remainingSeconds)}</strong>
                  <span>{session.isExpired ? '本次翻阅已结束' : '剩余时间'}</span>
                </div>
              </header>
              {session.isExpired && (
                <FeedbackMessage state="empty" message="本次限时翻阅已结束。当前卡片仍可翻转，但不能继续切换。" />
              )}
              <article
                className={`cram-flashcard ${session.snapshot.flipped ? 'is-flipped' : ''}`}
                tabIndex={0}
                onKeyDown={(event) => {
                  if (event.key === 'Enter' || event.key === ' ') {
                    event.preventDefault();
                    session.toggleFlipped();
                  }
              ...
              >
                <div className="cram-card-face">
                  <p className="workbench-eyebrow">{currentCard.importance === 'critical' ? '核心重点' : '知识模块'}</p>
                  <h3>{currentCard.title}</h3>
                  <p>
                    {session.snapshot.flipped
                      ? (currentCard.examRelevance ?? currentCard.contentSummary)
                      : (currentCard.contentSummary ?? currentCard.examRelevance)}
                  </p>
                </div>
                <footer>
                  <div className="cram-source-list">
                    {currentCard.sources.map((source) => (
                      <span key={source.kind}>
                        {SOURCE_LABELS[source.kind]} {source.kind === 'knowledge_module' ? '' : `${source.count} 条`}
                      </span>
                    ))}
                  </div>
                  <button type="button" onClick={session.toggleFlipped} aria-pressed={session.snapshot.flipped}>
                    {session.snapshot.flipped ? '查看摘要面' : '翻转查看考点'}
                  </button>
                </footer>
              </article>
              <div className="cram-controls">
                <button type="button" onClick={() => go(-1)} disabled={!canNavigate || session.currentIndex === 0}>
                  上一张
                </button>
                <button
                  type="button"
                  onClick={() => go(1)}
                  disabled={!canNavigate || session.currentIndex >= session.snapshot.cardIds.length - 1}
                >
                  下一张
                </button>
