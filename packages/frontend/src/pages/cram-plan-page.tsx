...
function targetPath(
  ...
  if (suggestion.targetType === 'weak_point' || suggestion.targetType === 'mistake')
    return `/exams/${encodedExamId}/mistakes`;
...
export function CramPlanPage({ semesterId, onSemesterError }: CramPlanPageProps) {
  ...
  if (!semesterId) {
    return (
      <div className="page">
        <FeedbackMessage state="empty" message="请先创建或选择当前学期，才能查看冲刺计划。" />
      </div>
    );
  }
  ...
  const materialPath = currentData?.exam.courseInstanceId
    ? `/materials?courseInstanceId=${encodeURIComponent(currentData.exam.courseInstanceId)}`
    : '/materials';
  ...
  return (
    <div className="page cram-plan-page">
      ...
      {currentData && (
        <>
          <ExamContextNav
            examId={currentData.exam.id}
            courseInstanceId={currentData.exam.courseInstanceId}
            active="cram_plan"
          />
          <header className="card cram-plan-header">
            ...
            <p className="text-muted">
              建议只读取当前已确认考试的课程任务、练习与错题事实；所有入口仅导航，不会自动写入完成状态。
            </p>
          </header>
          {currentData.exam.confirmationStatus !== 'confirmed' && (
            <FeedbackMessage state="empty" message="请先确认考试信息，确认后才能生成冲刺计划。" />
          )}
          ...
          {currentData.exam.confirmationStatus === 'confirmed' &&
            currentData.plan?.availability === 'available' &&
            daysWithSuggestions.length === 0 && (
          ...
          {currentData.exam.confirmationStatus === 'confirmed' &&
            ...
            daysWithSuggestions.length > 0 && (
              <section className="cram-plan-days" aria-label="每日冲刺建议" data-testid="cram-plan-days">
                <p className="text-muted">
                  距离考试 {currentData.plan.daysUntilExam} 天 · 每项建议只提供前往既有学习页面的入口。
                </p>
                    ...
                    <ul>
                      {day.suggestions.map((suggestion) => (
                        <li key={suggestion.id} className="cram-plan-suggestion">
                          <div>
                            <strong>{SOURCE_LABELS[suggestion.sourceKind]}</strong>
                          ...
                          <Link
                            ...
                            to={targetPath(
                              ...
                              semesterId,
                              currentData.exam.id,
                              currentData.exam.courseInstanceId
                            )}
                          >
                            前往复习
                          </Link>
                ...
                <section className="card cram-plan-manual">
                  <h2>需要补充复习？</h2>
                  <ManualLinks
                    practicePath={practicePath}
                    mistakePath={mistakePath}
...
function ManualLinks({
  ...
  return (
    <div className="cram-manual-links">
      <Link className="button-link" to={practicePath}>
        开始练习
      </Link>
      <Link className="button-link" to={mistakePath}>
        查看错题本
      </Link>
      <Link className="button-link" to={cramPath}>
        临考速背
      </Link>
