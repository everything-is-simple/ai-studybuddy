...
import {
  ...
  createInvalidKeyFactoryAttempt,
  createShapedKeyFactoryAttempt,
  createFingerprintMismatchAttempt,
  createIntegrityBindingMismatchAttempt,
  createTransparentProxyApprovalFixture,
  createRecord,
  createSentinel,
  assertFixedError,
} from './helpers/trusted-approval-fixture.mjs';
  ...
  assert.throws(
    () =>
      fixture.verifier.verifyTrustedApproval({
        ...
        recordBytes: fixture.recordBytes,
        signatureBytes: fixture.signatureBytes,
        expected: { ...fixture.expected, purpose: 'T02-R2', sentinel },
      }),
  ...
  assert.throws(
    () =>
      fixture.verifier.verifyTrustedApproval({
        algorithm: 'Ed25519',
        recordBytes: fixture.recordBytes,
        signatureBytes: Buffer.alloc(63),
        expected: fixture.expected,
      }),
...
test('synthetic anchor requires a real Ed25519 KeyObject and synthetic input getters remain redacted', () => {
  ...
  assert.throws(
    () => createInvalidKeyFactoryAttempt()(),
    (error) => assertFixedError(error, 'TRUSTED_ANCHOR_INVALID', sentinel)
  );
  assert.throws(
    () => createShapedKeyFactoryAttempt()(),
    (error) => assertFixedError(error, 'TRUSTED_ANCHOR_INVALID', sentinel)
  ...
  assert.doesNotThrow(() =>
    proxiedFixture.verifier.verifyTrustedApproval({
      algorithm: 'Ed25519',
      recordBytes: proxiedFixture.recordBytes,
      signatureBytes: proxiedFixture.signatureBytes,
      expected: proxiedFixture.expected,
    })
  );
  const fixture = createApprovalFixture();
  const input = new Proxy(
  ...
  assert.throws(
    () => fixture.verifier.verifyTrustedApproval(input),
    (error) => assertFixedError(error, 'TRUSTED_APPROVAL_RECORD_INVALID', sentinel)
  );
});
...
test('synthetic trust anchor rejects fingerprint mismatch and mismatched integrity binding', () => {
  ...
  assert.throws(
    () => createIntegrityBindingMismatchAttempt()(),
    (error) => assertFixedError(error, 'TRUSTED_ANCHOR_BINDING_MISMATCH', sentinel)
  );
});
...
test('synthetic verifier recreates fixed errors for expected and integrity Proxy failures', () => {
  ...
  assert.throws(
    () =>
      expectedFixture.verifier.verifyTrustedApproval({
        algorithm: 'Ed25519',
        recordBytes: expectedFixture.recordBytes,
        signatureBytes: expectedFixture.signatureBytes,
  ...
  const hostileIntegrity = new Proxy(
    {},
    {
      get() {
  ...
  assert.throws(
    () => createApprovalFixture({ integrity: hostileIntegrity }),
    (error) => assertFixedError(error, 'TRUSTED_VERIFIER_INTEGRITY_UNPROVEN', getterSentinel)
  );
