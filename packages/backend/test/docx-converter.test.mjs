...
const { DocxConverter } = await import(pathToFileURL(path.join(__dirname, '../dist/adapters/docx-converter.js')).href);
...
function makeParagraph(text, visuals = []) {
  const visualXml = visuals.map((v) => `<w:${v}/>`).join('');
...
test('DocxConverter extracts Chinese text and counts embedded visuals', async () => {
  ...
  assert.ok(
    result.metadata.embeddedVisualCount >= 3,
    `应至少统计 3 个视觉对象，得到 ${result.metadata.embeddedVisualCount}`
  );
});
