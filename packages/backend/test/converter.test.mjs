...
const pdfSamplePath = 'I:\\ai-studybuddy-composer\\pdf\\pdf-parse-demo\\samples\\test.pdf';
const imageSamplePath = 'I:\\ai-studybuddy-composer\\ocr\\RapidOCR\\samples\\test.jpg';
...
test('PdfConverter extracts text from a text-based PDF', async () => {
  ...
  assert.ok(result.metadata && result.metadata.pageCount && result.metadata.pageCount > 0, '应能读取页数');
  assert.ok(result.metadata && typeof result.metadata.charCount === 'number', '应返回字符数');
});
...
test('OcrConverter recognizes image when rapidocr-onnxruntime is available', async () => {
  ...
  try {
    ...
    assert.ok(typeof result.metadata === 'object' && typeof result.metadata.charCount === 'number', '应返回字符数');
  } catch (error) {
