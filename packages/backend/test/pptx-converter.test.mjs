...
const { PptxConverter } = await import(pathToFileURL(path.join(__dirname, '../dist/adapters/pptx-converter.js')).href);
...
function makeSlideXml(num, text, opts = {}) {
  ...
  const pictureXml = hasPicture ? `<p:pic><p:blipFill><a:blip r:embed="rId2"/></a:blipFill></p:pic>` : '';
  const textXml = text ? `<a:p><a:r><a:t>${text}</a:t></a:r></a:p>` : '';
...
async function buildPptxBuffer(slides) {
  ...
  for (const { num, text, picture } of slides) {
    zip
      .folder('ppt')
      .folder('slides')
      .file(`slide${num}.xml`, makeSlideXml(num, text, { picture }));
