...
async function startBackend(t) {
  ...
  const child = spawn(process.execPath, ["dist/server.js"], {
    cwd: backendDir,
    env: { ...process.env, APP_DATA_ROOT: dataRoot, BACKEND_HOST: "127.0.0.1", BACKEND_PORT: String(port), AI_PROVIDERS: "", AI_API_KEY: "" },
...
async function json(port, method, pathname, body) {
  const response = await fetch(`http://127.0.0.1:${port}${pathname}`, { method, headers: body ? { "content-type": "application/json" } : undefined, body: body ? JSON.stringify(body) : undefined });
  return { status: response.status, json: await response.json() };
}

async function readySemester(port) {
  const response = await json(port, "POST", "/api/dev/init-semester", { studentName: "T07", semesterCode: `t07-${crypto.randomUUID()}`, teachingStartDate: "2026-02-20", teachingEndDate: "2026-06-30" });
...
test("S2 uploads text, converts it, and preserves normalized text when AI is unavailable", async (t) => {
  ...
  for (let attempt = 0; attempt < 220; attempt += 1) {
    ...
    if (response.json.data.status === "pending_quality_check") { detail = response.json.data; break; }
    await new Promise((resolve) => setTimeout(resolve, 100));
  }
...
test("S2 worker runOnce generates notes, modules, list metadata, and study evidence with mock AI", async (t) => {
  ...
  try {
    ...
    completedEvent = db.prepare("SELECT event_type, evidence_ref, quality_gate, source_confidence FROM study_events WHERE event_type = 'material_note_completed'").get();
  } finally {
    db.close();
  }
  ...
  try {
    const statusEvent = dbAfterUpdate.prepare("SELECT evidence_ref, quality_gate FROM study_events WHERE event_type = 'knowledge_module_status_changed'").get();
