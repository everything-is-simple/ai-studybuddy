...
async function initializeReadySemester(port) {
  const result = await requestJson(port, 'POST', '/api/dev/init-semester', {
    studentName: 'T04-Cram',
    semesterCode: `t04-cram-${crypto.randomUUID()}`,
...
function seedModule(dataRoot, semesterId, courseId, overrides = {}) {
  ...
  try {
    ...
    db.prepare(
      ...
      courseId,
      overrides.title ?? '模块',
      overrides.importance ?? 'medium',
      '禁止返回的资料原文',
      overrides.contentSummary ?? '可安全展示的摘要',
      overrides.examRelevance ?? '可安全展示的考点',
      now,
      now
    );
    return id;
...
test('T04 cram cards aggregate same-course S2/S4 signals deterministically without leaking source facts or writing', async (t) => {
  ...
  const weakNewer = seedModule(backend.dataRoot, semesterId, course.id, {
    title: '薄弱新模块',
    importance: 'critical',
  });
  const mistakeModule = seedModule(backend.dataRoot, semesterId, course.id, { title: '错题模块', importance: 'high' });
  const baselineModule = seedModule(backend.dataRoot, semesterId, course.id, {
    title: '基础模块',
    importance: 'critical',
  });
  ...
  const result = await requestJson(
    backend.port,
    'GET',
    `/api/assessment-attempts/${exam.id}/cram-cards?semesterId=${semesterId}`
  );
  assert.equal(result.status, 200, result.text);
  assert.equal(result.json.success, true);
  assert.equal(result.json.data.courseInstanceId, course.id);
  assert.deepEqual(
    result.json.data.cards.map((card) => card.knowledgeModuleId),
    [weakNewer, weakOlder, mistakeModule, baselineModule]
  );
  assert.deepEqual(result.json.data.cards[0].sources, [
    { kind: 'weak_point', count: 3 },
    { kind: 'knowledge_module', count: 1 },
  ]);
  assert.deepEqual(result.json.data.cards[2].sources, [
    { kind: 'knowledge_module', count: 1 },
...
test('T04 cram cards reject unconfirmed and other-semester exams and allow an empty safe-card response', async (t) => {
  ...
  const pendingResult = await requestJson(
    backend.port,
    'GET',
    `/api/assessment-attempts/${pending.id}/cram-cards?semesterId=${semesterId}`
  );
  ...
  const emptyResult = await requestJson(
    backend.port,
    'GET',
    `/api/assessment-attempts/${confirmed.id}/cram-cards?semesterId=${semesterId}`
  );
  ...
  const crossSemester = await requestJson(
    backend.port,
    'GET',
    `/api/assessment-attempts/${otherExam.id}/cram-cards?semesterId=${semesterId}`
  );
