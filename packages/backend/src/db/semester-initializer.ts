...
export function validateSemesterInitializationInput(input: SemesterInitializationInput): SemesterInitializationInput {
  ...
  if (studentName.length > 100 || semesterCode.length > 100) {
    throw new SemesterInitializationError('INVALID_PARAMS', 400, 'studentName and semesterCode must be ≤ 100 chars');
  }
  ...
  if (input.teachingStartDate > input.teachingEndDate) {
    throw new SemesterInitializationError('INVALID_DATE_RANGE', 400, 'teachingStartDate must be ≤ teachingEndDate');
  }

  if (input.finalArchiveDate && input.finalArchiveDate < input.teachingEndDate) {
    throw new SemesterInitializationError('INVALID_DATE_RANGE', 400, 'finalArchiveDate must be ≥ teachingEndDate');
  }
  ...
  return {
    ...
    teachingStartDate: input.teachingStartDate,
    teachingEndDate: input.teachingEndDate,
    ...(input.finalArchiveDate === undefined ? {} : { finalArchiveDate: input.finalArchiveDate }),
  };
}
...
export function initializeSemester(
  ...
  try {
    ...
    const duplicate = globalDb.prepare('SELECT id FROM semesters WHERE semester_code = ?').get(input.semesterCode) as
      { id: string } | undefined;
    if (duplicate) {
      throw new SemesterInitializationError(
    ...
    phase = 'global_write';
    globalDb.transaction(() => {
      const existingStudent = globalDb!.prepare('SELECT id FROM students WHERE name = ?').get(input.studentName) as
        { id: string } | undefined;
      const studentId = existingStudent?.id ?? crypto.randomUUID();

      if (!existingStudent) {
        globalDb!
          .prepare('INSERT INTO students (id, name, created_at, updated_at) VALUES (?, ?, ?, ?)')
    ...
    const markReady =
      ...
      ((db: DatabaseType, id: string, timestamp: string) => {
        db.prepare('UPDATE semesters SET ready = 1, updated_at = ? WHERE id = ?').run(timestamp, id);
      });
    phase = 'mark_ready';
