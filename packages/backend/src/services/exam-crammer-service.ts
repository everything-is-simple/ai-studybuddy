...
type PaperRow = {
  ...
  status: MockExamPaperDetailDto['status'];
  title: string;
  question_count: number;
  time_limit_seconds: number;
  total_points: number;
  difficulty_preference: MockExamDifficultyPreference;
...
type CramCardRow = {
  ...
  content_summary: string | null;
  exam_relevance: string | null;
  weak_point_evidence_count: number | null;
  weak_point_latest_at: string | null;
  mistake_count: number;
  mistake_error_count: number;
  mistake_latest_at: string | null;
};
...
export class ExamCrammerService {
  ...
  openReadySemesterDb(semesterId: string): DatabaseType {
    ...
    if (!fs.existsSync(getSemesterDbPath(semesterId)))
      throw new ExamCrammerError('SEMESTER_DB_NOT_FOUND', 500, '学期数据库不存在');
  ...
  private requireConfirmedAssessment(
    db: DatabaseType,
    assessmentId: string,
    courseId: string
  ): Record<string, unknown> {
    const row = db
      .prepare('SELECT * FROM assessment_attempts WHERE id = ? AND course_instance_id = ?')
      .get(assessmentId, courseId) as Record<string, unknown> | undefined;
    if (!row) throw new ExamCrammerError('ASSESSMENT_ATTEMPT_NOT_FOUND', 404, '考试不存在或不属于该课程');
    if (row.confirmation_status !== 'confirmed')
      throw new ExamCrammerError('ASSESSMENT_NOT_CONFIRMED', 409, '只有已确认考试才能生成模拟卷');
    return row;
  }
  ...
  getCramCards(semesterValue: unknown, assessmentValue: unknown): CramFlashcardResponseDto {
    ...
    try {
      const assessmentRelation = db
        .prepare('SELECT course_instance_id FROM assessment_attempts WHERE id = ?')
        .get(assessmentAttemptId) as { course_instance_id: string } | undefined;
      ...
      const cards = rows
        .map((row): CramFlashcardDto & { priority: number; evidence: number; latestAt: string } => {
          ...
          const latestAt =
            priority === 0 ? (row.weak_point_latest_at ?? '') : priority === 1 ? (row.mistake_latest_at ?? '') : '';
          return {
            id: row.id,
            knowledgeModuleId: row.id,
            title: text(row.title),
            importance: (importanceOrder[row.importance] === undefined
              ? 'low'
              : row.importance) as CramFlashcardDto['importance'],
      ...
      const rows = db
        .prepare(
          'SELECT id, title, importance, difficulty, content_summary, exam_relevance, source_evidence ' +
            'FROM knowledge_modules WHERE course_instance_id = ? AND id IN (' +
            placeholders +
            ')'
        )
        .all(courseId, ...ids) as ModuleRow[];
      if (rows.length !== ids.length)
        throw new ExamCrammerError('KNOWLEDGE_MODULE_NOT_FOUND', 404, '知识模块不存在或不属于该课程');
  ...
  private sourceSummary(db: DatabaseType, courseId: string, modules: ModuleRow[], assessmentName: string) {
    const weakPointCount = Number(
      (
        db
          .prepare("SELECT count(*) AS count FROM weak_points WHERE course_instance_id = ? AND status = 'active'")
          .get(courseId) as { count: number }
      ).count
    );
    const activeMistakeCount = Number(
      (
        db
          .prepare("SELECT count(*) AS count FROM mistakes WHERE course_instance_id = ? AND status <> 'mastered'")
          .get(courseId) as { count: number }
      ).count
    );
  ...
  private parseQuestion(raw: RawQuestion, allowed: Set<string>): GeneratedQuestion {
    ...
    if (!QUESTION_TYPES.includes(type))
      throw new ExamCrammerError('MOCK_EXAM_GENERATION_FAILED', 502, 'AI 生成题型不符合要求');
    const stem = text(raw.stem);
    if (!stem || stem.length > 2000)
      throw new ExamCrammerError('MOCK_EXAM_GENERATION_FAILED', 502, 'AI 生成题干不符合要求');
    const moduleId = text(raw.knowledge_module_id);
    if (!allowed.has(moduleId))
      throw new ExamCrammerError('MOCK_EXAM_GENERATION_FAILED', 502, 'AI 生成题目关联了未选择的知识模块');
    const difficulty = (text(raw.difficulty) || 'medium') as PracticeDifficulty;
    if (!DIFFICULTIES.includes(difficulty))
      throw new ExamCrammerError('MOCK_EXAM_GENERATION_FAILED', 502, 'AI 生成题目难度不符合要求');
    const pointValue =
      raw.point_value === undefined
        ? 1
        : positiveInteger(raw.point_value, 'MOCK_EXAM_GENERATION_FAILED', 'AI 生成分值不符合要求');
    ...
    if (type === 'fill_blank') {
      if (raw.options !== undefined && raw.options !== null)
        throw new ExamCrammerError('MOCK_EXAM_GENERATION_FAILED', 502, '填空题不能包含选项');
      const correct = text(raw.correct_answer);
      if (!correct) throw new ExamCrammerError('MOCK_EXAM_GENERATION_FAILED', 502, '填空题答案不能为空');
      const acceptable =
        raw.acceptable_answers == null
          ? null
          : parseStringArray(raw.acceptable_answers, '填空题可接受答案格式不符合要求');
      return {
        type,
        stem,
        options: null,
        ...
        sourceEvidence: text(raw.source_evidence) || null,
        pointValue,
      };
    }
    const options = parseStringArray(raw.options, '选择题选项格式不符合要求');
    if (options.length < 4 || options.length > 5)
      throw new ExamCrammerError('MOCK_EXAM_GENERATION_FAILED', 502, '选择题选项数量不符合要求');
    const correct =
      type === 'single_choice'
        ? normalizeLetters(raw.correct_answer, 1, 1)
        : normalizeLetters(raw.correct_answer, 2, Math.min(4, options.length));
    if (correct.split(',').some((item) => OPTIONS.indexOf(item) >= options.length))
      throw new ExamCrammerError('MOCK_EXAM_GENERATION_FAILED', 502, '选择题答案超出选项范围');
    return {
      type,
      stem,
      options,
  ...
  private mapAiError(error: unknown): never {
    ...
    if (error instanceof AllProvidersCoolingDownError)
      throw new ExamCrammerError(error.code, 503, 'AI Provider 暂时都在冷却中，请稍后再试');
    if (error instanceof AllProvidersFailedError)
      throw new ExamCrammerError(error.code, 502, 'AI 题目生成失败，请稍后再试');
    if (error instanceof AiProviderError)
      throw new ExamCrammerError(error.code, error.code === 'AI_NOT_CONFIGURED' ? 503 : 502, error.message);
  ...
  private parseGenerated(content: string, count: number, allowed: Set<string>): GeneratedQuestion[] {
    ...
    try {
      parsed = JSON.parse(sanitizeJson(content)) as { questions?: RawQuestion[] };
    } catch {
      throw new ExamCrammerError('MOCK_EXAM_GENERATION_FAILED', 502, 'AI 输出无法解析为题目 JSON');
    }
    if (!Array.isArray(parsed.questions) || parsed.questions.length !== count)
      throw new ExamCrammerError('MOCK_EXAM_GENERATION_FAILED', 502, 'AI 输出题目数量不符合要求');
  ...
  private parseRequest(input: CreateMockExamPaperRequest) {
    ...
    const assessmentAttemptId = requiredUuid(
      body.assessmentAttemptId,
      'MISSING_REQUIRED_FIELD',
      'assessmentAttemptId 不能为空'
    );
    const ids = body.knowledgeModuleIds == null ? null : body.knowledgeModuleIds;
    if (ids !== null && (!Array.isArray(ids) || ids.length < 1 || ids.length > 30))
      throw new ExamCrammerError('MOCK_EXAM_INPUT_INVALID', 400, 'knowledgeModuleIds 必须选择 1 到 30 个模块');
    const moduleIds =
      ids === null
        ? null
        : [
            ...new Set(
              ids.map((id) => requiredUuid(id, 'MOCK_EXAM_INPUT_INVALID', 'knowledgeModuleIds 必须全部是有效 UUID'))
            ),
          ];
    if (moduleIds && ids !== null && moduleIds.length !== ids.length)
      throw new ExamCrammerError('MOCK_EXAM_INPUT_INVALID', 400, 'knowledgeModuleIds 不能重复');
    const count = body.questionCount ?? 10;
    if (typeof count !== 'number' || !Number.isInteger(count) || count < 5 || count > 50)
      throw new ExamCrammerError('MOCK_EXAM_INPUT_INVALID', 400, 'questionCount 必须是 5 到 50 的整数');
    const difficulty = body.difficultyPreference ?? 'mixed';
    if (!PREFERENCES.includes(difficulty))
      throw new ExamCrammerError('MOCK_EXAM_INPUT_INVALID', 400, 'difficultyPreference 非法');
    const timeLimit =
      body.timeLimitSeconds == null
        ? count * 90
        : positiveInteger(body.timeLimitSeconds, 'MOCK_EXAM_INPUT_INVALID', 'timeLimitSeconds 必须为正整数');
  ...
  async createMockExamPaper(input: CreateMockExamPaperRequest): Promise<MockExamPaperDetailDto> {
    ...
    try {
      ...
      if (modules.length === 0)
        throw new ExamCrammerError('KNOWLEDGE_MODULE_NOT_FOUND', 404, '该课程尚无可用于生成模拟卷的知识模块');
        ...
        generated!.forEach((question, index) =>
          insert.run(
            ...
            question.sourceEvidence ??
              modules.find((item) => item.id === question.knowledgeModuleId)?.source_evidence ??
              null,
            question.pointValue,
            index + 1,
            timestamp
  ...
  getPaper(semesterIdValue: unknown, paperIdValue: unknown): MockExamPaperDetailDto {
    ...
    try {
      ...
      const questions = db
        .prepare('SELECT * FROM mock_exam_questions WHERE paper_id = ? ORDER BY question_order')
        .all(paperId) as QuestionRow[];
  ...
  startAttempt(input: { semesterId: unknown; paperId: unknown }): MockExamAttemptDetailDto {
    ...
    try {
      ...
      db.prepare(
        ...
        id,
        paper.id,
        paper.course_instance_id,
        paper.assessment_attempt_id,
        timestamp,
        paper.total_points,
        timestamp,
        timestamp
  ...
  getAttempt(semesterIdValue: unknown, attemptIdValue: unknown): MockExamAttemptDetailDto {
    ...
    try {
      ...
      const questions = db
        .prepare('SELECT * FROM mock_exam_questions WHERE paper_id = ? ORDER BY question_order')
        .all(row.paper_id) as QuestionRow[];
      return {
  ...
  private normalizeChoice(value: unknown, optionsJson: string | null, multiple: boolean): string | null {
    ...
    if (!multiple && parts.length !== 1)
      throw new ExamCrammerError('MOCK_EXAM_ANSWER_INVALID', 400, '单选题答案格式不合法');
    const unique = [...new Set(parts)].sort();
    if (unique.some((item) => !allowed.includes(item)))
      throw new ExamCrammerError('MOCK_EXAM_ANSWER_INVALID', 400, '答案选项不在题目范围内');
  ...
  private grade(
    ...
    submitted?: { answer: unknown; timeSpentSeconds: number | null }
  ): MockExamAnswerResultDto & { answerOrder: number; timeSpentSeconds: number | null } {
    ...
    return {
      ...
      studentAnswer: stored,
      correctAnswer: question.correct_answer,
      isCorrect: correct,
      scoreAwarded: correct ? Number(question.point_value) : 0,
      pointValue: Number(question.point_value),
      explanation: question.explanation,
      knowledgeModuleId: question.knowledge_module_id,
      answerOrder: Number(question.question_order),
      timeSpentSeconds,
    };
  }
  ...
  private parseSubmit(input: SubmitMockExamAttemptRequest) {
    ...
    if (!Array.isArray(input?.answers))
      throw new ExamCrammerError('MOCK_EXAM_SUBMIT_INPUT_INVALID', 400, 'answers 必须是数组');
    const seen = new Set<string>();
    const answers = input.answers.map((raw) => {
      if (!raw || typeof raw !== 'object')
        throw new ExamCrammerError('MOCK_EXAM_SUBMIT_INPUT_INVALID', 400, 'answers 格式不合法');
      const questionId = requiredUuid(raw.questionId, 'MOCK_EXAM_SUBMIT_INPUT_INVALID', 'questionId 必须是有效 UUID');
      if (seen.has(questionId))
        throw new ExamCrammerError('MOCK_EXAM_SUBMIT_INPUT_INVALID', 400, 'answers 不能包含重复题目');
      seen.add(questionId);
      const time =
        raw.timeSpentSeconds == null
          ? null
          : nonNegativeInteger(
              raw.timeSpentSeconds,
              'MOCK_EXAM_SUBMIT_INPUT_INVALID',
    ...
    try {
      return db.transaction(() => {
        ...
        if (attempt.status !== 'in_progress')
          throw new ExamCrammerError('MOCK_EXAM_ATTEMPT_STATE_INVALID', 409, '当前模拟考状态不允许提交');
        const questions = db
          .prepare('SELECT * FROM mock_exam_questions WHERE paper_id = ? ORDER BY question_order')
          .all(attempt.paper_id) as QuestionRow[];
        const paper = db
          .prepare('SELECT question_count, time_limit_seconds FROM mock_exam_papers WHERE id = ?')
          .get(attempt.paper_id) as { question_count: number; time_limit_seconds: number } | undefined;
        if (!paper || questions.length === 0 || questions.length !== Number(paper.question_count))
          throw new ExamCrammerError('MOCK_EXAM_QUESTION_MISMATCH', 409, '模拟卷题目数量不一致');
        const byId = new Map(questions.map((question) => [question.id, question]));
        for (const item of valid.answers)
          if (!byId.has(item.questionId))
            throw new ExamCrammerError('MOCK_EXAM_QUESTION_MISMATCH', 400, '答案引用了不属于该模拟考的题目');
        ...
        const timestamp = this.now();
        const insert = db.prepare(
          'INSERT INTO mock_exam_answers (id, attempt_id, question_id, student_answer, is_correct, score_awarded, time_spent_seconds, answer_order, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)'
        );
        graded.forEach((item) =>
          insert.run(
            this.id(),
        ...
        db.prepare(
          "UPDATE mock_exam_attempts SET status = 'graded', submitted_at = ?, graded_at = ?, total_score = ?, correct_rate = ?, overtime = ?, total_duration_seconds = ?, updated_at = ? WHERE id = ?"
        ).run(
          timestamp,
          timestamp,
          totalScore,
        ...
        graded.forEach((item) => {
          const items = grouped.get(item.knowledgeModuleId) ?? [];
          items.push(item);
          grouped.set(item.knowledgeModuleId, items);
        });
        const analyses: MockExamModuleAnalysisDto[] = [];
        const insertAnalysis = db.prepare(
          'INSERT INTO mock_exam_module_analyses (id, attempt_id, knowledge_module_id, question_count, correct_count, score_awarded, total_points, correct_rate, weak_signal, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
        );
        for (const [moduleId, items] of grouped) {
          ...
          analyses.push({
            ...
            correctCount: moduleCorrect,
            scoreAwarded: moduleScore,
        ...
        db.prepare(
          "INSERT INTO study_events (id, course_instance_id, source_system, event_type, title, workload_minutes, evidence_ref, parent_visible, occurred_at, created_at) VALUES (?, ?, 'S5', 'mock_exam_completed', '模拟考已完成', ?, ?, 1, ?, ?)"
        ).run(
          this.id(),
          attempt.course_instance_id,
          Math.ceil(valid.totalDurationSeconds / 60),
          'mock_exam_attempt:' + attemptId,
          timestamp,
          timestamp
        );
        return {
