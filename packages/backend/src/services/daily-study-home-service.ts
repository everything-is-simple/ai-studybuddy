  ...
  getHome(semesterId: unknown, date: unknown): DailyStudyHomeDto {
    ...
    try {
      ...
      const todayTasks = openTasks
        .filter((task) => task.deadline_at && timeDay(task.deadline_at) === date)
        .map(mapTask);
      const tomorrowTasks = openTasks
        .filter((task) => task.deadline_at && timeDay(task.deadline_at) === addCalendarDays(date, 1))
        .map(mapTask);
      ...
      `
          ...
          .all(date) as Array<{ id: string; name: string; exam_at: string; course_name: string }>
      ).map((exam): DailyStudyHomeExamDto => ({
        id: exam.id,
        name: exam.name,
      ...
      ).map((material): DailyStudyHomeMaterialDto => ({
        id: material.id,
      ...
      if (pendingQualityMaterials[0]) {
        ...
        nextAction = {
          ...
          path: `/courses/${openTasks.find((task) => task.id === todayTasks[0].id)?.course_instance_id}`,
        };
      } else if (tomorrowTasks[0]) {
        nextAction = {
          kind: 'tomorrow_task',
          title: tomorrowTasks[0].title,
          path: `/courses/${openTasks.find((task) => task.id === tomorrowTasks[0].id)?.course_instance_id}`,
        };
      } else if (errorReviews[0]) {
        nextAction = {
          kind: 'error_review',
          title: errorReviews[0].title,
          path: `/courses/${openTasks.find((task) => task.id === errorReviews[0].id)?.course_instance_id}`,
        };
      } else if (upcomingExams[0]) {
        nextAction = {
          kind: 'upcoming_exam',
          title: `准备考试：${upcomingExams[0].name}`,
          path: `/exams/${upcomingExams[0].id}`,
        };
      }
