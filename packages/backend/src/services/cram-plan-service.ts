...
export class CramPlanService {
  ...
  getCramPlan(semesterValue: unknown, assessmentValue: unknown): CramPlanResponseDto {
    ...
    try {
      ...
      if (assessment.confirmation_status !== 'confirmed')
        throw new ExamCrammerError('ASSESSMENT_NOT_CONFIRMED', 409, '只有已确认考试才能生成冲刺计划');
      ...
      const availability: CramPlanResponseDto['availability'] =
        daysUntilExam < 0 ? 'ended' : daysUntilExam > 7 ? 'not_started' : 'available';
      ...
      for (const row of weakRows) {
        candidates.push({
          id: `weak:${row.id}`,
          priority: 2,
          reason: `薄弱点已有 ${Number(row.evidence_count)} 条证据`,
          sourceKind: 'weak_point',
          sourceId: row.id,
          targetType: 'weak_point',
          targetId: row.id,
          rank: -Number(row.evidence_count),
          dueAt: '',
          recentAt: row.latest_detected_at,
        });
      }
      ...
      for (const row of mistakeRows) {
        candidates.push({
          id: `mistake:${row.id}`,
          priority: 3,
          reason: `错题累计 ${Number(row.error_count)} 次错误`,
          sourceKind: 'mistake',
          sourceId: row.id,
          targetType: 'mistake',
          targetId: row.id,
          rank: -Number(row.error_count),
          dueAt: '',
          recentAt: row.latest_error_at,
        });
      }
      ...
      for (const row of practiceRows) {
        if (Number(row.correct_rate) < 0.8)
          candidates.push({
            id: `practice:${row.id}`,
            priority: 4,
            reason: `已完成练习正确率 ${Math.round(Number(row.correct_rate) * 100)}%，建议针对性复盘`,
            sourceKind: 'practice_performance',
            sourceId: row.id,
            targetType: 'practice_history',
            targetId: row.id,
            rank: Number(row.correct_rate),
            dueAt: '',
            recentAt: row.graded_at,
          });
      }
      const moduleCount = (
        db
      ...
      candidates.sort(
        (a, b) =>
          ...
          a.rank - b.rank ||
          (a.priority === 1 ? a.dueAt.localeCompare(b.dueAt) : b.recentAt.localeCompare(a.recentAt)) ||
          a.id.localeCompare(b.id)
      );
      const days = Array.from({ length: Math.max(1, daysUntilExam + 1) }, (_, index) => ({
        date: addCalendarDays(today, index),
        suggestions: [] as CramPlanSuggestionDto[],
      }));
      for (const [index, candidate] of candidates.entries()) {
        days[index % days.length].suggestions.push({
          id: candidate.id,
