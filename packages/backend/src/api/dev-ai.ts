...
function validateAiRequest(body: unknown): { ok: false; error: { status: number; response: ApiError } } | { ok: true; request: AiRequest } {
  ...
  if (!validTaskTypes.includes(taskType)) {
    return {
      ok: false,
      error: { status: 400, response: errorResponse("AI_TASK_TYPE_INVALID", `taskType 必须是 ${validTaskTypes.join("/")} 之一`) },
    };
  }
