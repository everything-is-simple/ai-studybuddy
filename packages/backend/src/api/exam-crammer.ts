...
router.get('/mock-exam-papers/:id', (req: Request, res: Response) => {
  try { return res.json(ok(service.getPaper(req.query.semesterId, req.params.id))); }
  catch (error) { return handle(error, res); }
});

router.post('/mock-exam-papers/:id/attempts', (req: Request, res: Response) => {
  try { return res.status(201).json(ok(service.startAttempt({ semesterId: req.body?.semesterId, paperId: req.params.id }))); }
  catch (error) { return handle(error, res); }
});

router.get('/mock-exam-attempts/:id', (req: Request, res: Response) => {
  try { return res.json(ok(service.getAttempt(req.query.semesterId, req.params.id))); }
  catch (error) { return handle(error, res); }
});
