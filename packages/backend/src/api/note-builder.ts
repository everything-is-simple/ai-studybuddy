...
function handle(error: unknown, res: Response): Response {
  if (error instanceof multer.MulterError && error.code === 'LIMIT_FILE_SIZE')
    return res.status(413).json(fail('FILE_TOO_LARGE', '文件大小超过 10MB 限制'));
...
router.post('/materials/:id/retry-conversion', (req: Request, res: Response) => {
  try {
    res.json(ok(service.retry(req.body.semesterId, req.params.id, 'material_convert')));
  } catch (error) {
    handle(error, res);
  }
});
router.post('/materials/:id/retry-ai-generation', (req: Request, res: Response) => {
  try {
    res.json(ok(service.retry(req.body.semesterId, req.params.id, 'note_generate')));
  } catch (error) {
    handle(error, res);
  }
});
...
router.get('/knowledge-modules', (req: Request, res: Response) => {
  try {
    res.json(
      ok(
        service.listKnowledgeModules(req.query.semesterId, req.query.courseInstanceId, {
...
router.patch('/knowledge-modules/:id', (req: Request, res: Response) => {
  try {
    res.json(
      ok(
        service.updateKnowledgeModule({
          semesterId: req.body.semesterId,
          id: req.params.id,
          learnStatus: req.body.learnStatus,
          importance: req.body.importance,
          difficulty: req.body.difficulty,
          examRelevance: req.body.examRelevance,
        })
      )
    );
  } catch (error) {
