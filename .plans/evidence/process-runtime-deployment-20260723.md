...
- 远端最终哈希以实时 `git ls-remote --heads origin master` 的当前结果为准；不在仓库文档写死最终哈希，避免提交自身改变哈希后文档过期。
...
- `git push origin master`：已成功。
- 随后实时 `git ls-remote --heads origin master`：已与本地 `HEAD` 匹配；具体最终哈希见交付说明中的实时输出。
