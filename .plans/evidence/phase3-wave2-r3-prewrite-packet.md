...
```
审查人：用户（一切尽在掌握的独立审查角色）
审查日期：2026-08-06
R3-prewrite 结论：☑ 签收（无 P1 未闭合）
签收确认：T04-3 受控恢复写入实现、8 步持久化状态机（状态文件/中断标记/重启默认拒绝/完整性复验）、
           Gate B（ACL 误报已澄清无 P1）、日志允许字段契约均已复核通过；
           Wave 1 隔离根 + Wave 2 选项 C 部署格式隔离安装根真实模拟演练证据齐全。
签收后授权范围（精确机器/时间窗口）：⬜ 待用户另行明确（目标机器/安装实例/data 根/时间窗口
           5 项批准仍未填写——真实写入前必须补齐，且执行时须再采集停止证据）
审查人签名：用户确认（2026-08-06）
```

> **签收记录（2026-08-06）**：用户以独立审查人身份确认 R3-prewrite 签收。本签收解除 T04-3 的“未签收不得真实写入”门禁，但**目标机精确批准（机器/实例/data 根/时间窗口）与停止证据仍未采集**；在此两项齐全前，T04-3 继续对真实数据保持 `RESTORE_WRITE_DISABLED`（fail-closed）。
