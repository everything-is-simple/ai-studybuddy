...
> **状态**：历史 T02 总计划。用户于 2026-07-27 的全面暂停已于 2026-07-28 被有限撤销；只允许按 `.plans/phase3-reopen-high-weight-20260728-plan.md` 对 11 个高权重必做项另建、审查和实施。T02A–T02G 已完成各自批准范围；T02 总体、真实 Windows ACL/backup/restore、restore 写入、用户电脑实机证据与全部修复仍未完成。
> **计划分支**：`codex/phase3-t02-security-privacy-baseline-audit-plan`
> **基线**：2026-07-25 的最新 `origin/master`（创建 worktree 时为 `a0ba36e`）。
> **历史暂停边界与现行替代**：T02G 是 2026-07-27 暂停前最后一个进入 `origin/master` 的切片，其合成夹具通过范围不变，不得外推为真实 ACL、真实 backup/restore、restore 写入、用户电脑实机验收、T02 总体或 Phase 3 完成。2026-07-28 起，只有新恢复计划列出的 11 项可以进入后续独立实施计划；其余范围继续不启动。
...
4. Phase 3 曾启动 T02 计划及 T02A–T02G 独立切片；用户于 2026-07-27 曾全面冻结后续工作，并于 2026-07-28 仅对新恢复计划列明的 11 项有限重新开放。
...
## 7. 原后续实施切片建议（暂停保留，不授权执行）

以下内容仅保留为历史审计分解，不构成继续实施授权。自 2026-07-27 起，不再拆分或实施任何 T02 后续切片；未来如需恢复，必须由用户再次明确批准、按届时最新事实重新审查，并另建独立计划。发现 P0 时仍应停止普通流程并单独报告，但不得据此自动恢复 Phase 3。
...
## 9. 未来恢复后的验证命令（当前不得据此启动）

仅当用户未来明确恢复并批准新的独立计划时，所有会写运行数据的命令才可在新的仓库外隔离 `APP_DATA_ROOT` 下执行。最低验证集：
...
本次暂停登记只运行文档治理、`git diff --check`、状态检查和必要的只读一致性搜索；不运行会写数据的业务测试、真实外部服务试验、ACL、备份、恢复或部署脚本。
...
- 状态表述：通过。**本计划曾获批准，T02A–T02G 已完成各自切片范围；用户已暂停 T02 剩余工作。暂停不表示 T02、Phase 3、安全修复、真实机器操作或上线验收完成。**
- 恢复门禁：当前不再执行或新建 T02 后续切片。未来只有用户再次明确批准并创建新的独立计划后，才能依据届时风险重新排序；如发现 P0，只报告并停止普通流程。
